...
class AstarGridTests(TestCaseMixin):
    ...
    def test(self):
        ...
        for x in range(5):
            w.set(x, 2, 0)
        w.set(2, 2, 1)
        path: list[Cell] = astar(GridNav(w, GridConnectivity.Four), Cell(0, 0), Cell(4, 0))
...
class AstarGridUnreachableTests(TestCaseMixin):
    ...
    def test(self):
        w: Grid2D = new(3, 1, 1)
        w.set(1, 0, 0)
        path: list[Cell] = astar(GridNav(w, GridConnectivity.Four), Cell(0, 0), Cell(2, 0))
...
class AstarGraphTests(TestCaseMixin):
    ...
    def test(self):
        ...
        g.add_edge(0, 3, 10)
        empty_h: list[int] = []
        path: list[int] = astar(GraphNav(g, empty_h), 0, 3)
...
class DijkstraGraphTests(TestCaseMixin):
    ...
    def test(self):
        ...
        g.add_edge(2, 1, 1)
        empty_h: list[int] = []
        path: list[int] = dijkstra(GraphNav(g, empty_h), 0, 1)
