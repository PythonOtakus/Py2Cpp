...
> **状态**：P0.5/P1b + P1 + P2 + **P3**（`grid2d` / `graph` / `navigate` + ``Navigatable``）已落地；P3b `spatial_hash`、P4 见 §7。  
...
| 类别 | 说明 |
|------|------|
| 通用图算法库 | 网络流、JPS、flow field 等；**仅** `navigate.py` 提供对 ``Navigatable`` 的 ``astar`` / ``dijkstra`` |
...
| 场景 | 使用现有模块 | `alg` 补充 |
|------|-------------|-----------|
| 动态数组、栈数组 | `list`、`array`、`stack_array` | — |
...
| **块大小** | `ChunkDeque` 默认 **`block_size=512`**（构造可改；须写清文档） |
...
| 空切片 ``st[l:l]`` | 聚合**单位元**（``AggMode`` 决定：``Sum``→0，``Min``→极大，``Max``→极小） |

闭区间旧写法 ``query(l, r)``（含 ``r``）→ 改为 ``st[l:r+1]``。

### ``AggMode`` 枚举

- 构造：``SegTree(n, AggMode.Sum)``、``SparseTable(data, AggMode.Min)``；
- **禁止**字符串 ``"sum"`` / ``"min"``；
...
- ``MonoQueue``：``push`` / ``pop`` / ``min`` 或 ``max``（``is_min`` 构造参数）。
...
**非** CPython ``util/deque`` 全套。默认 ``block_size=512``；块内 ``list[T]`` + ``@boxing`` 块级双向链（``chunk_deque.py`` 单类实现）。
...
py2cpp/
  alg/
    ...
    agg_mode.py          # 区间聚合 ``AggMode``（``SegTree`` / ``SparseTable``）
    seg_tree.py          # 线段树（点修 + 区间聚合，``SegTree``）
    sparse_table.py      # 稀疏表（静态 RMQ）
    trie.py              # 字典树（str 前缀）
    ac_auto.py           # AC 自动机（``ACAuto``，多模式匹配）
    protocols.py         # ``Navigatable[N]`` 寻路协议
    grid2d.py            # ``Grid2D`` / ``WalkGrid`` / ``GridNav`` + ``GridConnectivity``
    graph.py             # ``AdjList`` / ``GraphNav``
    navigate.py          # ``astar`` / ``dijkstra``（仅 ``Navigatable`` 参数）
...
| **线段树** | `seg_tree` | 区间最值/和、点修 | `__init__(n, AggMode)`, `st[i]`/`st[l:r]`/`st[i]=v`, `len`/`in` |
| **稀疏表** | `sparse_table` | 静态 RMQ | `__init__(arr, AggMode.Min/Max)`, `st[i]`/`st[l:r]`, `len`/`in` |
...
``T`` 须支持 ``+`` / ``-``（``@protocol Complex`` 及其实现，如 ``int`` / ``float``）。
...
class IndexedHeap[T: Comparable & DictKey]:
...
### 5.10 寻路协议 `Navigatable` + `navigate`

```python
@protocol
class Navigatable[N: DictKey]:
  type Node = N
...
``GridNav`` / ``GraphNav`` 满足协议；算法层**只**接收 ``Navigatable``。

### 5.11 二维网格 `Grid2D` / `GridConnectivity`（游戏）
...
class Grid2D:
  ...
  # width / height / get / set / in_bounds / fill / copy

class Grid2D:
  # ``0`` 墙，``>= 1`` 边权；``walkable`` / ``get_width`` / ``get_height`` / ``get`` / ``set`` / ``fill``
