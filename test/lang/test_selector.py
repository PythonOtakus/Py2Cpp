...
class SelectSliceOmitTests(TestCaseMixin):
    ...
    def test(self):
        ...
        allMembers: list[Member] = t.select('.members[:]')
        self.assertEqual(len(allMembers), 3)
        self.assertEqual(allMembers[0].name, 'amy')
        self.assertEqual(allMembers[2].name, 'c')
...
class SelectMultiPathTests(TestCaseMixin):
    ...
    def test(self):
        ...
        t2: Team = Team(name='beta')
        org.teams.append(t2)
        viaBracket: list[str] = org.select('.teams[0, 1].name')
        self.assertEqual(len(viaBracket), 2)
        self.assertEqual(viaBracket[0], 'alpha')
        self.assertEqual(viaBracket[1], 'beta')
        viaProj: list[str] = org.select('.(teams[0], teams[1]).name')
        self.assertEqual(viaProj, viaBracket)
...
class SelectBindRefTests(TestCaseMixin):
    ...
    def test(self):
        org: Org = buildOrg()
        viaSemi: list[str] = org.select('.teams[0]:$t; $t.name')
        self.assertEqual(len(viaSemi), 1)
        self.assertEqual(viaSemi[0], 'alpha')
        viaInline: list[str] = org.select('.teams[0]:$t.members[1].name')
        self.assertEqual(len(viaInline), 1)
        self.assertEqual(viaInline[0], 'bob')
...
class SelectFilterBindRefTests(TestCaseMixin):
    ...
    def test(self):
        org: Org = buildOrg()
        names: list[str] = org.select('.teams[0]:$t.members{.score > $t.minScore}.name')
        self.assertEqual(len(names), 1)
