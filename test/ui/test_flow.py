...
class FlowRuntimeLinearTests(TestCaseMixin):
    ...
    def test(self):
        ...
        g.connect(g.find_pin_on_node(evt, 'then', FlowPinKind.ExecOut), g.find_pin_on_node(fire, 'execute', FlowPinKind.ExecIn))
        g.connect(g.find_pin_on_node(three, 'Return Value', FlowPinKind.DataOut), g.find_pin_on_node(fire, 'shots', FlowPinKind.DataIn))
        rt: FlowRuntime = new()
...
class FlowRuntimeBranchTests(TestCaseMixin):
    ...
    def test(self):
        ...
        g.connect(g.find_pin_on_node(evt, 'then', FlowPinKind.ExecOut), g.find_pin_on_node(branch, 'execute', FlowPinKind.ExecIn))
        g.connect(g.find_pin_on_node(yes, 'Return Value', FlowPinKind.DataOut), g.find_pin_on_node(branch, 'condition', FlowPinKind.DataIn))
        g.connect(g.find_pin_on_node(branch, 'OnTrue', FlowPinKind.ExecOut), g.find_pin_on_node(fire, 'execute', FlowPinKind.ExecIn))
        g.connect(g.find_pin_on_node(three, 'Return Value', FlowPinKind.DataOut), g.find_pin_on_node(fire, 'shots', FlowPinKind.DataIn))
        rt: FlowRuntime = new()
        rt.run_from_event(g, cat, host, evt)
        self.assertEqual(host.hp, 97)
...
class FlowRuntimeForLoopTests(TestCaseMixin):
    ...
    def test(self):
        ...
        fire: int = host.add_kind('host.fire', 400.0, 0.0)
        three: int = host.add_kind('host.three', 200.0, 100.0)
        g.connect(g.find_pin_on_node(evt, 'then', FlowPinKind.ExecOut), g.find_pin_on_node(loop, 'execute', FlowPinKind.ExecIn))
        g.connect(g.find_pin_on_node(three, 'Return Value', FlowPinKind.DataOut), g.find_pin_on_node(loop, 'count', FlowPinKind.DataIn))
        g.connect(g.find_pin_on_node(loop, 'LoopBody', FlowPinKind.ExecOut), g.find_pin_on_node(fire, 'execute', FlowPinKind.ExecIn))
        g.connect(g.find_pin_on_node(three, 'Return Value', FlowPinKind.DataOut), g.find_pin_on_node(fire, 'shots', FlowPinKind.DataIn))
        rt: FlowRuntime = new()
        rt.run_from_event(g, cat, host, evt)
...
class FlowRuntimeHost:
    ...
    def flow_invoke_callable(self, method: str, graph: FlowGraph @ ref, node_id: int, rt: FlowRuntime @ ref) -> None:
        if method == 'fire':
