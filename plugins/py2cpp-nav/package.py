#!/usr/bin/env python3
"""将 Py2Cpp Navigation 扩展打包为 .vsix（纯 Python，无需 npm）。"""
...
def _collect_extension_files(root: Path) -> list[tuple[str, Path]]:
  ...
  if not any(name == "extension/out/extension.js" for name, _ in out):
    raise FileNotFoundError("缺少 out/extension.js")
  return out
...
def main(argv: list[str] | None = None) -> int:
  parser = argparse.ArgumentParser(description="打包 py2cpp-nav VSIX")
  ...
  print(f"已生成: {vsix}")
  print("安装: VS Code / CursorType → Extensions → … → Install from VSIX…")
  print(f"打包时间: {stamp}")
  return 0
