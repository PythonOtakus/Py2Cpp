...
from .kwargs_options import TRANSLATOR_ONLY_METHODS as _S0106_RESERVED_MEMBER_NAMES
...
_UNION_NAME_EXEMPT_S0104 = frozenset({'Result', 'Optional', 'IterResult'})
# CPython 同名异常：不强制 ``*Error``（含已以 Error 结尾的内建名）。
_CPYTHON_EXCEPTION_NAMES_S0104 = frozenset({
...
_S0104_SUFFIX_RULES: tuple[tuple[str, str, str], ...] = (
...
# ``@delegate`` 为函数定义（非 ClassDef），见 ``_check_s0104_naming_suffixes``
_SLICE_ARRAY_ANN_ROOTS = frozenset({'array', 'array2d', 'array3d', 'StackArray', 'StackArray2d', 'StackArray3d'})
_S0801_MIN_DISPATCH_BRANCHES = 3
_S0802_MIN_COMPARE_CHAIN_ARMS = 2
...
_S0101_GLOBAL_DUNDERS = frozenset({'__await__'})
...
class _Violation:
    ...
    node: ast.AST
    module_path: str
_S1101_EXEMPT = frozenset({'py2cpp/core/refcount.py', 'py2cpp/core/refcount'})

def _s1101_module_exempt(module_path: str) -> bool:
    norm = module_path.replace('\\', '/')
    if norm in _S1101_EXEMPT:
        return True
    return norm.endswith('/py2cpp/core/refcount.py') or norm.endswith('/py2cpp/core/refcount')
_S1102_EXEMPT = frozenset({'py2cpp/core/none.py', 'py2cpp/core/none'})

def _s1102_module_exempt(module_path: str) -> bool:
    norm = module_path.replace('\\', '/')
    if norm in _S1102_EXEMPT:
        return True
...
def check_strict_style(tr: TranslatorState):
    ...
    for module_path, tree in tr.module_asts.items():
        ...
        _check_s1001_overload(tr, module_path, tree, violations)
        _check_s1002_override_virtual(tr, module_path, violations)
        _check_s1003_method_decorator_conflicts(tr, module_path, violations)
        _check_static_virtual_protocol_rules(tr, module_path, violations)
        _check_s1004_unused_pep695_type_params(tree, module_path, violations)
        _check_s1005_reverse_self_dunder(tr, module_path, violations)
        _check_s0104_naming_suffixes(tree, module_path, violations)
    _check_s0501_private_field_accessor_pairs(tr, violations)
    _check_s0502_prefer_postsetter(tr, violations)
    _check_s0401_dataclass_container_optional(tr, violations)
    _check_s0201_imports(tr, violations)
    _check_s0604_slice_array_annotations(tr, violations)
    if not violations:
        return
...
def check_refcount_source_style(tr: TranslatorState) -> None:
    """S1101：全模块（含 ``test/fail/``）禁止显式 ``RefCount``。"""
    if not getattr(tr, 'strict', True):
        return
    violations: list[_Violation] = []
    for module_path, tree in tr.module_asts.items():
        ...
        if skip is not None and skip(module_path):
            continue
        norm = module_path.replace('\\', '/')
        if _s1101_module_exempt(module_path):
            continue
        source = _module_source(tr, module_path)
        lines = source.splitlines() if source else []
        checker = _StrictStyleChecker(tr, module_path, lines)
        for node in ast.walk(tree):
            if isinstance(node, ast.Subscript):
                checker._check_s1101_refcount_subscript(node)
            elif isinstance(node, ast.Call):
                checker._check_s1101_refcount_call(node)
        violations.extend(checker.violations)
    if not violations:
        return
    parts: list[str] = [f'发现 {len(violations)} 处 RefCount 用法违规（S1101）：']
    first_loc: SourceLocation | None = None
    for v in violations:
        loc = location_from_node(tr, v.node, module_path=v.module_path)
        prefix = loc.prefix() if loc is not None else '?'
        ...
        if first_loc is None and loc is not None:
            first_loc = loc
    raise TranslationError('\n'.join(parts), location=first_loc)
_S1102_PYNONE_NAME = re.compile('\\bPyNone\\b')

def check_pynone_source_style(tr: TranslatorState) -> None:
    """S1102：全模块（含 ``test/fail/``）禁止显式 ``PyNone``（查源码，非脱糖 AST）。"""
    if not getattr(tr, 'strict', True):
        return
    violations: list[_Violation] = []
    for module_path in tr.module_asts:
        ...
        if skip is not None and skip(module_path):
            continue
        if _s1102_module_exempt(module_path):
            continue
        source = _module_source(tr, module_path)
        if not source or _STRICT_OFF_FILE.search(source):
            continue
        seen_lines: set[int] = set()
        for m in _S1102_PYNONE_NAME.finditer(source):
            lineno = source.count('\n', 0, m.start()) + 1
            ...
            violations.append(_Violation(S1102, _strict_msg('PyNone', 'None', '源码', reason='``PyNone`` 为 C++ 基础设施类型名；注解与表达式须写 ``None``（如 ``IterResult[T, None]``）', example='got: -> IterResult[T, None]'), node, module_path))
    if not violations:
        return
    parts: list[str] = [f'发现 {len(violations)} 处 PyNone 用法违规（S1102）：']
    first_loc: SourceLocation | None = None
    for v in violations:
        loc = location_from_node(tr, v.node, module_path=v.module_path)
        prefix = loc.prefix() if loc is not None else '?'
...
def _s0104_enum_is_flag(node: ast.ClassDef) -> bool:
...
def _s0104_base_name(base: ast.expr) -> str | None:
...
def _s0104_collect_exception_names(tree: ast.Module) -> set[str]:
...
def _s0104_require_suffix(name: str, suffix: str) -> bool:
    return name.endswith(suffix)

def _check_s0104_naming_suffixes(tree: ast.Module, module_path: str, violations: list[_Violation]) -> None:
    """S0104：特殊类型 / Meta / Var / Mixin / Delegate / 异常类名后缀（见编码规范 §1.0.2）。"""
    ...
    if is_ffi_module_path(module_path.replace("\\", "/")):
        return
    exc_names = _s0104_collect_exception_names(tree)
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and has_named_decorator(node, 'delegate'):
            name = node.name
            if not _s0104_require_suffix(name, 'Delegate'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…Delegate', f'def {name}', reason='``@delegate`` 名须以 ``Delegate`` 结尾', example='got: @delegate def UIEventDelegate(): …'), node, module_path))
            continue
        if not isinstance(node, ast.ClassDef):
            continue
        name = node.name
        if has_enum_mro_decorator(node):
            if not _s0104_require_suffix(name, 'TypeEnum'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…TypeEnum', f'class {name}', reason='``@enum.mro`` 类名须以 ``TypeEnum`` 结尾', example='got: @enum.mro class PetKindTypeEnum(base=…):'), node, module_path))
            continue
        if has_named_decorator(node, 'enum'):
            if _s0104_enum_is_flag(node):
                if not _s0104_require_suffix(name, 'Flag'):
                    violations.append(_Violation(S0104, _strict_msg(name, f'{name}…Flag', f'class {name}', reason='``@enum(flag=True)`` 类名须以 ``Flag`` 结尾', example='got: @enum(flag=True) class PermFlag:'), node, module_path))
            elif not _s0104_require_suffix(name, 'Enum'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…Enum', f'class {name}', reason='``@enum`` 类名须以 ``Enum`` 结尾', example='got: @enum class ModeEnum:'), node, module_path))
            continue
        if has_union_mro_decorator(node):
            if not _s0104_require_suffix(name, 'TypeUnion'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…TypeUnion', f'class {name}', reason='``@union.mro`` 类名须以 ``TypeUnion`` 结尾', example='got: @union.mro class ExcTypeUnion(base=…):'), node, module_path))
            continue
        if has_named_decorator(node, 'union'):
            if name not in _UNION_NAME_EXEMPT_S0104 and not _s0104_require_suffix(name, 'Union'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…Union', f'class {name}', reason='``@union`` 类名须以 ``Union`` 结尾（豁免 Result / Optional / IterResult）', example='got: @union class MessageUnion:'), node, module_path))
            continue
        for deco, suffix, label in _S0104_SUFFIX_RULES:
            if has_named_decorator(node, deco):
                if not _s0104_require_suffix(name, suffix):
                    violations.append(_Violation(S0104, _strict_msg(name, f'{name}…{suffix}', f'class {name}', reason=f'``{label}`` 类名须以 ``{suffix}`` 结尾', example=f'got: {label} class Foo{suffix}:'), node, module_path))
                break
        if name in _CPYTHON_EXCEPTION_NAMES_S0104:
            continue
        if name in exc_names or name.endswith('Exception'):
            if not name.endswith('Error'):
                violations.append(_Violation(S0104, _strict_msg(name, f'{name}…Error', f'class {name}', reason='异常类除 CPython 同名外须以 ``Error`` 结尾', example='got: class EmptyError(Exception):'), node, module_path))
...
def _s0706_parse_range_start_stop_step(node: ast.Call) -> tuple[ast.expr, ast.expr, ast.expr | None] | None:
...
def _s0706_try_range_redundant_form(node: ast.Call) -> str | None:
    """``range(0, n)``/``, 1)`` → ``range(n)``；``range(a, b, 1)`` → ``range(a, b)``。"""
    parsed = _s0706_parse_range_start_stop_step(node)
    if parsed is None:
        return None
...
def _s0103_forward_param_names(node: ast.FunctionDef | ast.AsyncFunctionDef) -> set[str]:
    names: list[str] = []
...
def _s0103_all_param_names(node: ast.FunctionDef | ast.AsyncFunctionDef) -> set[str]:
    return {arg.arg for arg in node.args.posonlyargs + node.args.args + node.args.kwonlyargs}

def _s0103_body_single_forward_call(body: list[ast.stmt]) -> ast.Call | None:
    if len(body) != 1:
        return None
    stmt = body[0]
...
def _s0103_call_arg_names(call: ast.Call) -> set[str] | None:
    out: set[str] = set()
...
def _s0103_try_thin_param_forward(node: ast.FunctionDef | ast.AsyncFunctionDef, *, class_methods: set[str] | None, module_funcs: set[str], self_like: set[str]) -> str | None:
    ...
    if node.name.startswith('scan_test_') or node.name.startswith('scanTest'):
        return None
    call = _s0103_body_single_forward_call(node.body)
    if call is None:
        return None
    if isinstance(call.func, ast.Attribute):
        ...
        if recv == 'Self':
            if class_methods is None or callee not in class_methods:
                return None
            params = _s0103_forward_param_names(node)
            all_params = _s0103_all_param_names(node)
            arg_names = _s0103_call_arg_names(call)
            # 实例方法可把 ``self`` 转给 ``Self.静态(self, …)``；其余形参允许置换
            if arg_names is None or (arg_names != params and arg_names != all_params):
                return None
            wrong = f'`def {node.name}(…): return Self.{callee}(…)`'
...
def _s0102_alias_key_for_name(name: str) -> str | None:
...
def _cpp_style_alias_key(name: str) -> str | None:
    if _is_dunder(name):
        return None
    return _s0102_alias_key_for_name(name)
_S0106_MEMBER_HINTS: dict[str, str] = {'assign': '字段赋值请用译期 ``obj.assign(kw=…)``，就地拷贝请 ``copy_from(other)``', 'build': '对象构造请用译期 ``Type.build("…")``', 'select': '路径读请用译期 ``obj.select("…")``'}

def _s0106_translator_only_member_message(name: str, class_name: str, *, kind: str) -> str:
    hint = _S0106_MEMBER_HINTS.get(name, '改用其它名称')
...
def _s0102_cpp_style_message(name: str) -> str | None:
    ...
    if stripped != name and key == stripped:
        return _strict_msg(f'`def {name}`', f'Python/py2cpp 命名（{hint}）', '类/实例方法定义处', example=f'勿用前导 `_` 规避 {S0102}；应写 `def append` 等，勿 `def _{key}`', reason=f'去掉前导 `_` 后仍为 C++/STL 风格名 `{key}`')
    return _strict_msg(f'`def {name}`', f'Python/py2cpp 命名（{hint}）', '类/实例方法定义处', example=f'将 `def {name}(…)` 改为规范写法（见编码规范 {S0102} 表）', reason='勿引入 C++/STL 风格方法名')

def _s0101_dunder_alternative(attr: str) -> str:
    if attr == '__cmp__':
    ...
    return alts.get(attr, f'对应运算符或内建（见编码规范 §1.1 {S0101}）')

def _s0101_dunder_call_message(attr: str) -> str:
    alt = _s0101_dunder_alternative(attr)
...
def _s0301_empty_factory_message(factory: str) -> str:
...
def _s0303_no_empty_new_message(ann_root: str) -> str:
...
def _s0701_len_zero_suggestion(node: ast.Compare) -> str:
...
def _s0702_zero_slice_message(node: ast.Subscript) -> str:
...
def _s0703_len_minus_k_subscript_suggestion(name: str, k: int) -> str:
    return _strict_msg(f'`{name}[len({name}) - {k}]`', f'`{name}[-{k}]`', '负索引访问尾部元素时', example=f'`{name}[-{k}]` 勿 `{name}[len({name}) - {k}]`', reason='负下标更短且与 CPython 惯用法一致')

def _s0705_parse_index_compare(test: ast.expr) -> tuple[str, ast.cmpop, ast.expr] | None:
...
def _s0705_name_in_test(test: ast.expr, name: str) -> bool:
    for node in ast.walk(test):
        if isinstance(node, ast.Name) and node.id == name:
            return True
    return False

def _s0705_body_updates_test_bound_names(stmts: list[ast.stmt], index: str, test: ast.expr) -> bool:
    """条件里出现的其它变量若在体内被赋值/增强赋值（如 ``lo``/``hi``），则视为双指针循环。"""
    for stmt in stmts:
        if isinstance(stmt, ast.AugAssign):
            if isinstance(stmt.target, ast.Name):
                other = stmt.target.id
                if other != index and _s0705_name_in_test(test, other):
                    return True
            ...
            if _s0705_body_updates_test_bound_names(stmt.body, index, test):
                return True
            if _s0705_body_updates_test_bound_names(stmt.orelse, index, test):
                return True
            ...
            for handler in stmt.handlers:
                if _s0705_body_updates_test_bound_names(handler.body, index, test):
                    return True
            if _s0705_body_updates_test_bound_names(stmt.orelse, index, test):
                return True
            if _s0705_body_updates_test_bound_names(stmt.finalbody, index, test):
                return True
        elif isinstance(stmt, ast.Match):
            for case in stmt.cases:
                if _s0705_body_updates_test_bound_names(case.body, index, test):
                    return True
        elif isinstance(stmt, (ast.For, ast.While, ast.AsyncFor, ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            continue
    return False

def _s0705_aug_is_self_referential_step(aug: ast.AugAssign) -> bool:
    val = aug.value
    return isinstance(val, ast.Name) and isinstance(aug.target, ast.Name) and (val.id == aug.target.id)

def _s0705_aug_step_depends_on_index(aug: ast.AugAssign, index: str) -> bool:
...
def _s0705_collect_index_stores(stmts: list[ast.stmt], index: str) -> list[ast.stmt]:
    ...
    for stmt in stmts:
        if isinstance(stmt, ast.AugAssign):
            ...
            out.extend(_s0705_collect_index_stores(stmt.body, index))
            out.extend(_s0705_collect_index_stores(stmt.orelse, index))
        elif isinstance(stmt, ast.With):
            out.extend(_s0705_collect_index_stores(stmt.body, index))
            out.extend(_s0705_collect_index_stores(stmt.orelse, index))
        elif isinstance(stmt, (ast.Try, ast.TryStar)):
            out.extend(_s0705_collect_index_stores(stmt.body, index))
            for handler in stmt.handlers:
                out.extend(_s0705_collect_index_stores(handler.body, index))
            out.extend(_s0705_collect_index_stores(stmt.orelse, index))
            out.extend(_s0705_collect_index_stores(stmt.finalbody, index))
        elif isinstance(stmt, ast.Match):
            for case in stmt.cases:
                out.extend(_s0705_collect_index_stores(case.body, index))
...
def _s0705_step_is_negative(aug: ast.AugAssign) -> bool:
    val = aug.value
...
def _s0705_aug_matches_compare(op: ast.cmpop, aug: ast.AugAssign) -> bool:
    aop = aug.op
    ...
    if isinstance(op, (ast.Gt, ast.GtE)):
        ...
        if isinstance(aop, ast.Add):
            return _s0705_step_is_negative(aug)
    return False

def _s0705_while_range_message(index: str, bound: ast.expr, op: ast.cmpop, aug: ast.AugAssign) -> str:
...
def _s0705_len_bound_target_name(bound: ast.expr) -> str | None:
...
def _s0705_body_appends_to_name(stmts: list[ast.stmt], name: str) -> bool:
    ...
    for stmt in stmts:
        ...
        if isinstance(stmt, ast.If):
            if _s0705_body_appends_to_name(stmt.body, name) or _s0705_body_appends_to_name(stmt.orelse, name):
                return True
        elif isinstance(stmt, (ast.For, ast.While, ast.AsyncFor)):
            continue
    return False

def _s0705_try_index_while_range(node: ast.While) -> str | None:
    if isinstance(node.test, ast.BoolOp):
        return None
    parsed = _s0705_parse_index_compare(node.test)
    if parsed is None:
        return None
    index, op, bound = parsed
    len_name = _s0705_len_bound_target_name(bound)
    if len_name is not None and _s0705_body_appends_to_name(node.body, len_name):
        return None
    if _s0705_body_updates_test_bound_names(node.body, index, node.test):
        return None
    stores = _s0705_collect_index_stores(node.body, index)
    if len(stores) != 1 or not isinstance(stores[0], ast.AugAssign):
        return None
    aug = stores[0]
    if not isinstance(aug.target, ast.Name) or aug.target.id != index:
        return None
    if _s0705_aug_is_self_referential_step(aug):
        return None
    if _s0705_aug_step_depends_on_index(aug, index):
        return None
    if not _s0705_aug_matches_compare(op, aug):
        return None
    return _s0705_while_range_message(index, bound, op, aug)
...
def _s0704_binop_assign_message(node: ast.Assign) -> str:
...
def _s0303_scene_label(*, aug_op: str | None=None, in_aug_assign: bool=False, in_binop: bool=False, in_call_arg: bool=False, in_new_preferred: bool=False) -> str:
    if in_aug_assign:
        if aug_op:
...
def _s0303_reason_no_new_in_expr() -> str:
...
def _strict_msg(wrong: str, right: str, scene: str, *, example: str | None=None, reason: str | None=None) -> str:
    ...
    if example:
        msg += f'；例如 {example}'
    return msg
_s0303_msg_prefer = _strict_msg

def _s0303_priority_message(hint: str, *, scene: str | None=None, example: str | None=None) -> str:
...
def _s0303_preferred_init_hint(ann: ast.expr | None) -> str | None:
...
def _empty_literal_hint(ann: ast.expr | None) -> str | None:
    ...
    hint = _s0303_preferred_init_hint(ann)
    if hint == 'new()':
        return None
    return hint
...
def _s0303_exempt_new_receiver_class(info: ClassInfo) -> bool:
        ...
        if checker._current_func is not None:
            ...
            if ann is not None:
                ...
                if root is not None:
                    ...
                    if info is not None and info.is_union:
                        return info
    subject_cpp = _s0901_resolve_expr_cpp_type(checker, node.subject)
    if subject_cpp:
...
def _s0801_is_ord_single_char(expr: ast.expr) -> bool:
...
def _s0801_is_enum_member(expr: ast.expr) -> bool:
...
def _s0801_eq_rhs_ok(rhs: ast.expr) -> bool:
    if isinstance(rhs, ast.Constant):
        return True
    if _s0801_is_ord_single_char(rhs):
        return True
    return _s0801_is_enum_member(rhs)

def _s0801_in_container_ok(rhs: ast.expr) -> bool:
    if isinstance(rhs, ast.Constant) and isinstance(rhs.value, (str, bytes)):
        return True
    if isinstance(rhs, (ast.Set, ast.List, ast.Tuple)):
        return all((isinstance(elt, ast.Constant) or _s0801_is_enum_member(elt) or _s0801_is_ord_single_char(elt) for elt in rhs.elts))
    return False

def _s0801_compare_rhs_ok(op: ast.cmpop, rhs: ast.expr) -> bool:
    if isinstance(op, ast.Eq):
        return _s0801_eq_rhs_ok(rhs)
    if isinstance(op, ast.In):
        return _s0801_in_container_ok(rhs)
    return False

def _s0801_subject_from_test(test: ast.expr) -> ast.expr | None:
  ...
  """
    if isinstance(test, ast.BoolOp) and isinstance(test.op, ast.And):
        for value in test.values:
            subj = _s0801_subject_from_test(value)
            if subj is not None:
                return subj
        return None
    if isinstance(test, ast.Compare) and len(test.ops) == 1 and (len(test.comparators) == 1):
        op = test.ops[0]
        if isinstance(op, (ast.Eq, ast.In)) and _s0801_compare_rhs_ok(op, test.comparators[0]):
            return test.left
    return None

def _s0801_subjects_compatible(subjects: list[ast.expr]) -> bool:
    keys: list[str] = []
    for subj in subjects:
        try:
            ...
            return False
    return len(keys) >= _S0801_MIN_DISPATCH_BRANCHES and len(set(keys)) == 1

def _s0801_collect_elif_tests(node: ast.If) -> list[ast.expr]:
    tests = [node.test]
    cur = node.orelse
...
def _s0801_dispatch_ladder_msg(subject: str, branch_count: int) -> str:
...
def _s0801_report_compare_ladder(checker: _StrictStyleChecker, anchor: ast.AST, tests: list[ast.expr]) -> None:
    if len(tests) < _S0801_MIN_DISPATCH_BRANCHES:
        return
    subjects: list[ast.expr] = []
    for test in tests:
        subj = _s0801_subject_from_test(test)
        if subj is None:
            return
        subjects.append(subj)
    if not _s0801_subjects_compatible(subjects):
        return
    subject = ast.unparse(subjects[0])
    checker._add(S0801, anchor, _s0801_dispatch_ladder_msg(subject, len(tests)))

def _s0801_scan_if_orelse(checker: _StrictStyleChecker, orelse: list[ast.stmt]) -> None:
    if not orelse:
        return
    if len(orelse) == 1 and isinstance(orelse[0], ast.If):
        cur = orelse[0]
        while True:
            _s0801_scan_stmt_list(checker, cur.body)
            if not cur.orelse:
                return
            if len(cur.orelse) == 1 and isinstance(cur.orelse[0], ast.If):
                cur = cur.orelse[0]
                continue
            _s0801_scan_stmt_list(checker, cur.orelse)
            return
    _s0801_scan_stmt_list(checker, orelse)

def _s0801_scan_stmt_list(checker: _StrictStyleChecker, stmts: list[ast.stmt]) -> None:
    i = 0
    while i < len(stmts):
        if isinstance(stmts[i], ast.If):
            ...
            while j < len(stmts) and isinstance(stmts[j], ast.If):
                ...
                j += 1
            if len(group) >= _S0801_MIN_DISPATCH_BRANCHES:
                _s0801_report_compare_ladder(checker, group[0], [node.test for node in group])
            i = j
        else:
            i += 1
    for stmt in stmts:
        if isinstance(stmt, ast.If):
            _s0801_report_compare_ladder(checker, stmt, _s0801_collect_elif_tests(stmt))
            _s0801_scan_stmt_list(checker, stmt.body)
            _s0801_scan_if_orelse(checker, stmt.orelse)
        elif isinstance(stmt, (ast.For, ast.While, ast.AsyncFor)):
            _s0801_scan_stmt_list(checker, stmt.body)
            _s0801_scan_stmt_list(checker, stmt.orelse)
        elif isinstance(stmt, ast.With):
            _s0801_scan_stmt_list(checker, stmt.body)
        elif isinstance(stmt, (ast.Try, ast.TryStar)):
            _s0801_scan_stmt_list(checker, stmt.body)
            for handler in stmt.handlers:
                _s0801_scan_stmt_list(checker, handler.body)
            _s0801_scan_stmt_list(checker, stmt.orelse)
            _s0801_scan_stmt_list(checker, stmt.finalbody)
        elif isinstance(stmt, ast.Match):
            for case in stmt.cases:
                _s0801_scan_stmt_list(checker, case.body)
        elif isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            continue

def _s1201_for_target_name(target: ast.expr) -> str | None:
    if isinstance(target, ast.Name):
        return target.id
    return None

def _s1201_is_trivial_yield_delegation(node: ast.For) -> bool:
    ...
    if yval is None:
        return False
    tname = _s1201_for_target_name(node.target)
    if tname is None:
        return False
    return isinstance(yval, ast.Name) and yval.id == tname

def _s1201_yield_from_suggest(node: ast.For) -> str:
    try:
        return f'yield from {ast.unparse(node.iter)}'
    except Exception:
        return 'yield from <iterable>'

def _s1201_for_yield_msg(node: ast.For) -> str:
    wrong = f'for {ast.unparse(node.target)} in …: yield …'
    return _strict_msg(wrong, _s1201_yield_from_suggest(node), '生成器', reason='循环体仅 ``yield`` 循环变量时与 ``yield from`` 等价，状态机更简单', example=f'got: {wrong}')

def _s1201_record_for_violation(violations: list[_Violation], module_path: str, node: ast.For, *, in_async: bool) -> None:
    if in_async:
        return
    if not _s1201_is_trivial_yield_delegation(node):
        return
    violations.append(_Violation(S1201, _s1201_for_yield_msg(node), node, module_path))

def check_yield_from_for_style(tr: TranslatorState) -> None:
    """S1201：须在 ``expand_generators`` **之前**调用（同步 ``def`` 尚未脱糖为 ``*_generator``）。"""
    if not getattr(tr, 'strict', True):
        return
    violations: list[_Violation] = []
    for module_path, tree in tr.module_asts.items():
        ...
        class _S1201PreExpandWalker(ast.NodeVisitor):
            ...
            def visit_For(self, node: ast.For) -> None:
                _s1201_record_for_violation(violations, module_path, node, in_async=self.in_async)
                self.generic_visit(node)
        _S1201PreExpandWalker().visit(tree)
    if not violations:
        return
    parts: list[str] = [f'发现 {len(violations)} 处编码规范违规（可用 --no-strict 关闭）：']
    first_loc: SourceLocation | None = None
...
def _s0801_body_has_yield(body: list[ast.stmt]) -> bool:
    for node in ast.walk(ast.Module(body=body, type_ignores=[])):
        if isinstance(node, (ast.Yield, ast.YieldFrom)):
            return True
    return False

def _s0801_skip_function(node: ast.FunctionDef | ast.AsyncFunctionDef) -> bool:
    """``async def`` / 含 ``yield`` 的生成器：脱糖为状态机，``match`` 暂不与 S0801 同用。"""
    if isinstance(node, ast.AsyncFunctionDef):
        return True
    return _s0801_body_has_yield(node.body)

def _s0801_scan_function_body(checker: _StrictStyleChecker, body: list[ast.stmt], *, func: ast.FunctionDef | ast.AsyncFunctionDef | None=None) -> None:
    if func is not None and _s0801_skip_function(func):
        return
    _s0801_scan_stmt_list(checker, body)

def _s0802_single_compare_arm(expr: ast.expr, *, eq: bool) -> tuple[ast.expr, ast.expr] | None:
...
def _s0802_collect_compare_chain_arms(expr: ast.expr, bool_op: type[ast.boolop], *, eq: bool) -> list[tuple[ast.expr, ast.expr]] | None:
    arm = _s0802_single_compare_arm(expr, eq=eq)
    if arm is not None:
        return [arm]
    if isinstance(expr, ast.BoolOp) and isinstance(expr.op, bool_op):
        out: list[tuple[ast.expr, ast.expr]] = []
        for value in expr.values:
            part = _s0802_collect_compare_chain_arms(value, bool_op, eq=eq)
            if part is None:
                return None
            out.extend(part)
        return out
...
def _s0802_parse_compare_chain(expr: ast.expr, bool_op: type[ast.boolop], *, eq: bool) -> tuple[ast.expr, list[ast.expr]] | None:
    arms = _s0802_collect_compare_chain_arms(expr, bool_op, eq=eq)
    if arms is None or len(arms) < _S0802_MIN_COMPARE_CHAIN_ARMS:
        return None
    subject = arms[0][0]
    try:
...
def _s0802_rhs_char_code(rhs: ast.expr) -> str | None:
...
def _s0802_suggest_membership(subject: str, rhss: list[ast.expr], *, negate: bool) -> str:
    codes = [_s0802_rhs_char_code(rhs) for rhs in rhss]
...
def _s0802_compare_chain_msg(subject: str, rhss: list[ast.expr], *, eq_or: bool) -> str:
    suggest = _s0802_suggest_membership(subject, rhss, negate=not eq_or)
    if eq_or:
...
def _s0901_ann_is_char(ann: ast.expr | None) -> bool:
    if ann is None:
        return False
    try:
...
def _s0901_sig_for_func_node(checker: _StrictStyleChecker, node: ast.FunctionDef | ast.AsyncFunctionDef) -> FunctionSig | MethodSig | None:
...
def _s0901_param_cpp_type(checker: _StrictStyleChecker, func: ast.FunctionDef | ast.AsyncFunctionDef, name: str) -> str:
    ...
    if sig is not None:
        ...
        if pt:
            return pt
    for arg in (*func.args.posonlyargs, *func.args.args, *func.args.kwonlyargs):
        if arg.arg == name and _s0901_ann_is_char(arg.annotation):
            return cpp_ident('char')
    return ''

def _s0901_resolve_expr_cpp_type(checker: _StrictStyleChecker, expr: ast.expr) -> str:
    match expr:
        case ast.Name(id=name):
            if checker._current_func is not None:
                pt = _s0901_param_cpp_type(checker, checker._current_func, name)
                if pt:
                    return pt
        case ast.Subscript(value=value, slice=sl) if not isinstance(sl, ast.Slice):
            vt = _s0901_resolve_expr_cpp_type(checker, value)
            if is_str_type(vt):
                return cpp_ident('char')
    return checker.tr._infer_expr_cpp_type(expr)

def _s0901_expr_is_char_typed(checker: _StrictStyleChecker, expr: ast.expr) -> bool:
    ty = _s0901_resolve_expr_cpp_type(checker, expr)
    return bool(ty and is_char_type(ty))

def _s0901_char_literal_compare_msg(ch: str) -> str:
    if ch == "'":
        wrong = 'ch == "\'"'
        right = 'ch == ord("\'")'
    else:
...
def _s0902_ord_non_literal_msg(bad: str) -> str:
...
def _s0903_ctor_call_name(expr: ast.expr | None) -> str | None:
...
def _s0903_ann_text(ann: ast.expr | None) -> str:
    if ann is None:
        return ''
    try:
...
def _s0903_ann_matches_ctor(ann: ast.expr | None, ctor: str) -> bool:
    text = _s0903_ann_text(ann)
    if not text:
        return False
    if text == ctor:
        return True
...
def _s0903_primitive_convert_temp_msg(name: str, ctor: str, expr: str) -> str:
...
def _s0903_name_in_assign_target(target: ast.expr, name: str) -> bool:
    if isinstance(target, ast.Name):
        return target.id == name
    if isinstance(target, (ast.Tuple, ast.List)):
        return any((_s0903_name_in_assign_target(elt, name) for elt in target.elts))
    return False

def _s0903_assign_hits_in_stmt(stmt: ast.stmt, name: str) -> list[ast.AST]:
    if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
        return []
    hits: list[ast.AST] = []
    if isinstance(stmt, ast.AnnAssign):
        if _s0903_name_in_assign_target(stmt.target, name):
            hits.append(stmt)
    elif isinstance(stmt, ast.Assign):
        if any((_s0903_name_in_assign_target(t, name) for t in stmt.targets)):
            hits.append(stmt)
        ...
        for block in (stmt.body, stmt.orelse):
            for inner in block:
                hits.extend(_s0903_assign_hits_in_stmt(inner, name))
    elif isinstance(stmt, (ast.With, ast.AsyncWith)):
        for item in stmt.items:
            if item.optional_vars and _s0903_name_in_assign_target(item.optional_vars, name):
                hits.append(stmt)
        ...
        for child in ast.iter_child_nodes(stmt):
            if isinstance(child, ast.stmt):
                hits.extend(_s0903_assign_hits_in_stmt(child, name))
    return hits

def _s0903_assign_hits_in_function(func: ast.FunctionDef | ast.AsyncFunctionDef, name: str) -> list[ast.AST]:
    hits: list[ast.AST] = []
    for stmt in func.body:
        hits.extend(_s0903_assign_hits_in_stmt(stmt, name))
    return hits

def _s0903_check_primitive_convert_temp(checker: _StrictStyleChecker, node: ast.AnnAssign) -> None:
    if checker._s0303_in_desugar_host():
        return
    if node.value is None:
        return
    ctor = _s0903_ctor_call_name(node.value)
    if ctor is None or ctor not in _PRIMITIVE_CONVERT_CTORS:
        return
    if not _s0903_ann_matches_ctor(node.annotation, ctor):
        return
    if not isinstance(node.target, ast.Name):
        return
    func = checker._current_func
    if func is not None:
        hits = _s0903_assign_hits_in_function(func, node.target.id)
    ...
    try:
        ...
        arg_text = 'x'
    checker._add(S0903, node, _s0903_primitive_convert_temp_msg(node.target.id, ctor, arg_text))

def _s0310_ast_parents(root: ast.AST) -> dict[ast.AST, ast.AST]:
...
def _s0310_is_pure_forward_use(name_node: ast.Name, parents: dict[ast.AST, ast.AST]) -> bool:
    """``name`` 仅作为整表达式转发：``x = name`` / ``return name`` / ``fn(name)`` / ``fn(k=name)``。

    作为另一个 ``new(...)`` / ``new.meth(...)`` 的实参时不算（S0307 禁止嵌套 ``new``，须保留临时变量）。
    """
    ...
    if call is None:
        return False
    # 嵌套进另一个 new 构造：临时变量是 S0307 所需，非「仅为类型上下文」
    if _is_new_ctor_expr(call):
        return False
    return True

def _s0310_rewrite_new_to_cls(new_text: str, cls: str) -> str:
...
def _s0310_new_temp_msg(name: str, ann_text: str, new_text: str) -> str:
    cls = ann_text if ann_text else 'Cls'
    cls_form = _s0310_rewrite_new_to_cls(new_text, cls)
    return _strict_msg(
...
def _s0310_find_stmt_list(stmts: list[ast.stmt], target: ast.stmt) -> tuple[list[ast.stmt], int] | None:
    for i, stmt in enumerate(stmts):
        ...
        for child in nested:
            found = _s0310_find_stmt_list(child, target)
            if found is not None:
                return found
    return None

def _s0310_check_new_type_context_temp(checker: _StrictStyleChecker, node: ast.AnnAssign) -> None:
    """S0310：勿 ``sp: T = new(...); self.f = sp``（中间无其它语句）。"""
    if checker._s0303_in_desugar_host():
        return
    ...
    if func is None:
        return
    name = node.target.id
    hits = _s0903_assign_hits_in_function(func, name)
    if len(hits) != 1 or hits[0] is not node:
        return
    parents = _s0310_ast_parents(func)
    loads: list[ast.Name] = []
    ...
    if len(loads) > 1:
        return
    if not all(_s0310_is_pure_forward_use(n, parents) for n in loads):
        return
    ...
    for stmt in stmts[idx + 1 :]:
        ...
        if not loads_in_stmt:
            # 中间夹有其它语句（如 close 后再 return）——临时变量有序用途，不记 S0310
            return
        if not all(_s0310_is_pure_forward_use(n, parents) for n in loads_in_stmt):
            return
...
def _s1202_name_meaningfully_used(stmts: list[ast.stmt], name: str) -> bool:
    for stmt in stmts:
        if _s1202_is_discard_of_name(stmt, name):
            continue
        for node in ast.walk(stmt):
...
def _s1202_stmts_after(node: ast.Assign, func: ast.FunctionDef | ast.AsyncFunctionDef) -> list[ast.stmt]:
    hit = _s1202_find_stmts_after(node, func.body)
    return hit if hit is not None else []

def _s1202_nested_stmt_blocks(stmt: ast.stmt) -> list[list[ast.stmt]]:
...
def _s1202_find_stmts_after(node: ast.Assign, stmts: list[ast.stmt]) -> list[ast.stmt] | None:
    for i, stmt in enumerate(stmts):
        if stmt is node:
            return stmts[i + 1:]
        for block in _s1202_nested_stmt_blocks(stmt):
            hit = _s1202_find_stmts_after(node, block)
            if hit is not None:
                return hit
    return None

def _s1202_tuple_unpack_msg(name: str) -> str:
    return f'元组解包绑定 ``{name}`` 在后续未使用；未用槽位写 ``_``，或中间段写 ``*_``（如 ``{name}, *_ = …`` / ``*_, {name} = …``）'

def _s1202_check_tuple_unpack(checker: _StrictStyleChecker, node: ast.Assign) -> None:
    if checker._s0303_in_desugar_host():
        return
    func = checker._current_func
    if func is None:
    ...
    if not names:
        return
    rest = _s1202_stmts_after(node, func)
    for name in names:
        if _s1202_rhs_loads_name(node, name):
            continue
        if _s1202_name_meaningfully_used(rest, name):
            continue
        checker._add(S1202, node, _s1202_tuple_unpack_msg(name))

def _s1202_rhs_loads_name(node: ast.Assign, name: str) -> bool:
    for sub in ast.walk(node.value):
        if isinstance(sub, ast.Name) and sub.id == name and isinstance(sub.ctx, ast.Load):
            return True
    return False

def _s0803_match_missing_default_msg() -> str:
...
def _s0803_wildcard_with_guard_msg() -> str:
    return _strict_msg('末尾 `case _` 带 guard', 'case _:', '``match`` 最后一个 ``case``', reason='通配收尾不得写 ``case _ if …``；guard 请写在其它 ``case`` 上', example='match x:\n  case 1:\n    ...\n  case _:\n    ...')

def _s0803_union_case_ref(pattern: ast.pattern, *, subject_union: ClassInfo | None=None) -> tuple[str, str] | None:
...
def _s0803_union_case_uses_explicit_union_syntax(pattern: ast.pattern) -> bool:
    pat = pattern
    if isinstance(pat, ast.MatchAs) and pat.pattern is not None:
        pat = pat.pattern
...
def _s0803_union_branch_patterns(pattern: ast.pattern) -> list[ast.pattern]:
    if isinstance(pattern, ast.MatchOr):
        return list(pattern.patterns)
    return [pattern]

def _s0803_union_field_fully_bound(sub: ast.pattern) -> bool:
    """字段槽位须为名称绑定（``MatchAs``），勿用字面量约束（``MatchValue``）。"""
    return isinstance(sub, ast.MatchAs)

def _s0803_union_variant_fully_captured(info: ClassInfo, variant_name: str, pattern: ast.pattern, *, subject_union: ClassInfo | None=None) -> bool:
    ...
    if isinstance(pat, ast.MatchAs) and pat.pattern is not None:
        pat = pat.pattern
    ref = _s0803_union_case_ref(pat, subject_union=subject_union)
    ...
    for i, sub in enumerate(pat.patterns):
        if i >= len(variant.fields):
            return False
        if not _s0803_union_field_fully_bound(sub):
            return False
...
def _s0803_enum_members_from_pattern(pattern: ast.pattern) -> tuple[str, list[str]] | None:
...
def _s0803_is_exhaustive_enum_match_for(checker: _StrictStyleChecker, node: ast.Match) -> bool:
    """每个 ``@enum`` 成员至少一条无 guard 的 member ``case``。"""
    covered: dict[str, set[str]] = {}
    for case in node.cases:
        if case.guard is not None or is_wildcard_pattern(case.pattern):
            continue
        hit = _s0803_enum_members_from_pattern(case.pattern)
        if hit is None:
            return False
...
def _s0803_is_exhaustive_optional_match_for(checker: _StrictStyleChecker, node: ast.Match) -> bool:
    ...
    subject_cpp = _s0901_resolve_expr_cpp_type(checker, subject)
...
def _s0803_is_exhaustive_union_match_for(checker: _StrictStyleChecker, node: ast.Match) -> bool:
    ...
    for case in node.cases:
        if case.guard is not None or is_wildcard_pattern(case.pattern):
            continue
        for branch_pat in _s0803_union_branch_patterns(case.pattern):
            ref = _s0803_union_case_ref(branch_pat, subject_union=subject_union)
            if ref is None:
                return False
            ...
            if not _s0803_union_variant_fully_captured(union_info, variant_name, branch_pat, subject_union=subject_union):
                continue
...
def _s0803_check_match_default_case(checker: _StrictStyleChecker, node: ast.Match) -> None:
    if not node.cases:
        checker._add(S0803, node, _s0803_match_missing_default_msg())
        return
    last = node.cases[-1]
    if is_wildcard_pattern(last.pattern):
        if last.guard is not None:
            checker._add(S0803, last, _s0803_wildcard_with_guard_msg())
        return
    if _s0803_is_exhaustive_optional_match_for(checker, node):
        return
    if _s0803_is_exhaustive_union_match_for(checker, node):
        return
    if _s0803_is_exhaustive_enum_match_for(checker, node):
        return
    checker._add(S0803, node, _s0803_match_missing_default_msg())

def _s0804_slot_literal(sub: ast.pattern) -> object | None:
...
def _s0804_union_field_slots(pattern: ast.pattern, field_names: list[str]) -> list[object | None] | None:
    pat = pattern
    if isinstance(pat, ast.MatchAs) and pat.pattern is not None:
        pat = pat.pattern
    if isinstance(pat, ast.MatchValue):
    ...
    for i, sub in enumerate(pat.patterns):
        if i >= len(field_names):
            break
        slots[i] = _s0804_slot_literal(sub)
    for attr, sub in zip(pat.kwd_attrs, pat.kwd_patterns):
        if attr in field_names:
            slots[field_names.index(attr)] = _s0804_slot_literal(sub)
    return slots

def _s0804_earlier_pattern_shadows_later(slots_earlier: list[object | None], slots_later: list[object | None]) -> bool:
...
def _s0804_shadowed_case_msg(variant_name: str) -> str:
...
def _s0804_non_contiguous_variant_msg(variant_name: str) -> str:
...
def _s0804_single_variant_for_case(info: ClassInfo, case: ast.match_case) -> str | None:
    if is_wildcard_pattern(case.pattern):
        return None
    if isinstance(case.pattern, ast.MatchOr):
        return None
    ref = _s0803_union_case_ref(case.pattern, subject_union=info)
    if ref is None:
        return None
...
def _s0804_check_union_variant_contiguity(checker: _StrictStyleChecker, node: ast.Match, info: ClassInfo) -> None:
    last_index_by_variant: dict[str, int] = {}
    for idx, case in enumerate(node.cases):
        variant_name = _s0804_single_variant_for_case(info, case)
        if variant_name is None:
            continue
        prev = last_index_by_variant.get(variant_name)
        if prev is not None and idx != prev + 1:
            checker._add(S0804, case, _s0804_non_contiguous_variant_msg(variant_name))
        last_index_by_variant[variant_name] = idx

def _s0804_union_info_for_match(checker: _StrictStyleChecker, node: ast.Match) -> ClassInfo | None:
    from .union_expand import union_info_from_subject_cpp
    subject_union = _match_subject_union_info(checker, node)
...
def _s0805_optional_match_msg() -> str:
...
def _s0805_optional_variant_ctor_msg() -> str:
...
def _s0805_optional_identity_compare_msg(*, eq: bool) -> str:
    if eq:
...
def _s0805_optional_union_pattern(pattern: ast.pattern) -> ast.pattern | None:
    from .union_expand import parse_union_case_pattern
    if isinstance(pattern, ast.MatchOr):
        for branch in pattern.patterns:
            hit = _s0805_optional_union_pattern(branch)
            if hit is not None:
                return hit
        return None
    pat = pattern
...
def _s0805_check_optional_match_patterns(checker: _StrictStyleChecker, node: ast.Match) -> None:
    for case in node.cases:
        hit = _s0805_optional_union_pattern(case.pattern)
        if hit is not None:
            checker._add(S0805, hit, _s0805_optional_match_msg())

def _s0805_is_none_like_expr(node: ast.expr) -> bool:
...
def _s0805_optional_variant_attr(func: ast.expr) -> ast.Attribute | None:
...
def _s0805_check_optional_variant_ctor(checker: _StrictStyleChecker, node: ast.Call) -> None:
    attr = _s0805_optional_variant_attr(node.func)
    if attr is None:
        return
    if attr.attr == 'Some' or attr.attr == 'None_':
        checker._add(S0805, attr, _s0805_optional_variant_ctor_msg())

def _s0805_expr_is_optional_typed(checker: _StrictStyleChecker, expr: ast.expr) -> bool:
    ty = _s0901_resolve_expr_cpp_type(checker, expr)
    return bool(ty and is_optional_type(ty))

def _s0805_check_optional_identity_compare(checker: _StrictStyleChecker, node: ast.Compare) -> None:
    left: ast.expr = node.left
    for op, right in zip(node.ops, node.comparators):
        if isinstance(op, ast.Eq):
            if _s0805_expr_is_optional_typed(checker, left) and _s0805_is_none_like_expr(right):
                checker._add(S0805, node, _s0805_optional_identity_compare_msg(eq=True))
            elif _s0805_expr_is_optional_typed(checker, right) and _s0805_is_none_like_expr(left):
                checker._add(S0805, node, _s0805_optional_identity_compare_msg(eq=True))
        elif isinstance(op, ast.NotEq):
            if _s0805_expr_is_optional_typed(checker, left) and _s0805_is_none_like_expr(right):
                checker._add(S0805, node, _s0805_optional_identity_compare_msg(eq=False))
            elif _s0805_expr_is_optional_typed(checker, right) and _s0805_is_none_like_expr(left):
                checker._add(S0805, node, _s0805_optional_identity_compare_msg(eq=False))
        left = right

def _s0804_check_union_case_order(checker: _StrictStyleChecker, node: ast.Match) -> None:
    ...
    info = _s0804_union_info_for_match(checker, node)
    if info is None or is_optional_union_info(info):
        return
    _s0804_check_union_variant_contiguity(checker, node, info)
    ...
    for arm in arms:
        ...
        for i in range(len(cases)):
            if cases[i].guard is not None:
                continue
            slots_i = _s0804_union_field_slots(cases[i].pattern, field_names)
            if slots_i is None:
                continue
            for j in range(i + 1, len(cases)):
                slots_j = _s0804_union_field_slots(cases[j].pattern, field_names)
                if slots_j is None:
                    continue
                if _s0804_earlier_pattern_shadows_later(slots_i, slots_j):
                    checker._add(S0804, cases[j], _s0804_shadowed_case_msg(variant_name))

def _s0901_check_char_literal_compare(checker: _StrictStyleChecker, node: ast.Compare) -> None:
    ...
    for op, right in zip(node.ops, node.comparators):
        ...
        for char_expr, lit_expr in ((left, right), (right, left)):
            if not is_single_char_str_constant(lit_expr):
                continue
            if not _s0901_expr_is_char_typed(checker, char_expr):
                continue
            assert isinstance(lit_expr, ast.Constant) and isinstance(lit_expr.value, str)
            checker._add(S0901, node, _s0901_char_literal_compare_msg(lit_expr.value))
            return
        left = right

def _s0802_check_compare_chain(checker: _StrictStyleChecker, node: ast.BoolOp) -> None:
    if isinstance(node.op, ast.Or):
        parsed = _s0802_parse_compare_chain(node, ast.Or, eq=True)
        if parsed is not None:
            subject, rhss = parsed
            checker._add(S0802, node, _s0802_compare_chain_msg(ast.unparse(subject), rhss, eq_or=True))
        return
    if isinstance(node.op, ast.And):
        parsed = _s0802_parse_compare_chain(node, ast.And, eq=False)
        if parsed is None:
            return
        subject, rhss = parsed
        checker._add(S0802, node, _s0802_compare_chain_msg(ast.unparse(subject), rhss, eq_or=False))

def _s0701_is_len_zero_compare(node: ast.Compare) -> bool:
...
def _s0703_is_len_minus_k_subscript(node: ast.Subscript) -> int | None:
...
def _s0105_short_class_type_param_message(class_name: str, param: str) -> str:
    return _strict_msg(
...
_S0405_PROPERTY_STORAGE_SUFFIX = '__value'

def _s0405_display_field(name: str) -> str:
    if name.endswith(_S0405_PROPERTY_STORAGE_SUFFIX) and len(name) > len(_S0405_PROPERTY_STORAGE_SUFFIX):
        return name[:-len(_S0405_PROPERTY_STORAGE_SUFFIX)]
    return name

def _s0405_field_aliases(name: str) -> set[str]:
    names = {name}
    if name.endswith(_S0405_PROPERTY_STORAGE_SUFFIX) and len(name) > len(_S0405_PROPERTY_STORAGE_SUFFIX):
        names.add(name[:-len(_S0405_PROPERTY_STORAGE_SUFFIX)])
    else:
        names.add(name + _S0405_PROPERTY_STORAGE_SUFFIX)
    return names

def _s0405_skip_class(info: ClassInfo | None) -> bool:
    if info is None:
        return True
...
def _s0405_defaulted_fields(tr: Translator, info: ClassInfo) -> set[str]:
    names: set[str] = set()
    seen: set[int] = set()
    ...
    def walk(ci: ClassInfo) -> None:
        ...
        for fname in ci.field_defaults:
            ...
            if fname in ci.final_fields:
                continue
            names.update(_s0405_field_aliases(fname))
...
def _s0405_iter_assign_targets(target: ast.expr):
    if isinstance(target, (ast.Tuple, ast.List)):
        for elt in target.elts:
            yield from _s0405_iter_assign_targets(elt)
    else:
        yield target

def _s0602_subscript_slice_matches_class_type_params(sl: ast.expr, class_type_params: list[str]) -> bool:
...
def _s0602_allows_class_type_reference(hit: ast.expr, class_name: str, class_type_params: list[str]) -> bool:
    ...
    if not isinstance(hit.value, ast.Name) or hit.value.id != class_name:
        return False
    return not _s0602_subscript_slice_matches_class_type_params(hit.slice, class_type_params)

def _s0602_same_class_message(class_name: str, *, scene: str, attr: str | None=None) -> str:
    if attr is not None:
...
def _s0501_self_private_attr(node: ast.Attribute) -> str | None:
    if isinstance(node.value, ast.Name) and node.value.id == 'self':
        if node.attr.startswith('_') and (not node.attr.startswith('__')):
            return node.attr
    return None

def _s0501_instance_params(method: ast.FunctionDef) -> list[str]:
    return [a.arg for a in method.args.args if a.arg not in ('self', 'cls')]

def _s0501_decapitalize(s: str) -> str:
    if not s:
        return s
    return s[0].lower() + s[1:]

def _s0501_camel_prefix_stem(name: str, prefix: str) -> str | None:
    ...
    if not rest or not rest[0].isupper():
        return None
    return _s0501_decapitalize(rest)

def _s0501_getter_stem(name: str) -> str | None:
    for prefix in ('get', 'is'):
        stem = _s0501_camel_prefix_stem(name, prefix)
        if stem is not None:
            return stem
    if _s0501_camel_prefix_stem(name, 'set') is not None:
        return None
    return name

def _s0501_setter_stem(name: str) -> str | None:
    return _s0501_camel_prefix_stem(name, 'set')

def _s0501_try_pure_field_getter(method: ast.FunctionDef) -> str | None:
    if _is_dunder(method.name) or _is_property_accessor(method):
        return None
    if has_named_decorator(method, 'staticmethod'):
        return None
    if _s0501_instance_params(method):
        return None
    ...
    if isinstance(stmt.value, ast.Attribute):
        return _s0501_self_private_attr(stmt.value)
    return None

def _s0501_try_pure_field_setter(method: ast.FunctionDef) -> str | None:
    if _is_dunder(method.name) or _is_property_accessor(method):
        return None
    if has_named_decorator(method, 'staticmethod'):
        return None
    params = _s0501_instance_params(method)
    if len(params) != 1:
        return None
    if len(method.body) != 1:
        return None
    ...
    if not isinstance(tgt, ast.Attribute):
        return None
    field = _s0501_self_private_attr(tgt)
    if field is None:
        return None
    val = stmt.value
...
def _s0501_private_accessor_pair_msg(*, getter: str, setter: str, field: str) -> str:
...
def _check_s0501_private_field_accessor_pairs(tr: Translator, violations: list[_Violation]) -> None:
    skip = getattr(tr, 'skip_cached_analysis_module', None)
    for info in tr.classes.values():
        ...
        for method in _iter_class_methods(info):
            field = _s0501_try_pure_field_getter(method)
            if field is not None:
                getters_by_field[field] = method
                stem = _s0501_getter_stem(method.name)
                if stem is not None:
                    getters_by_stem[stem] = method
                continue
            field = _s0501_try_pure_field_setter(method)
            if field is not None:
                setters_by_field[field] = method
                stem = _s0501_setter_stem(method.name)
                if stem is not None:
        ...
        for field, getter in getters_by_field.items():
            ...
            if setter is None:
                g_stem = _s0501_getter_stem(getter.name)
            ...
            if key in reported:
                continue
            reported.add(key)
            msg = _s0501_private_accessor_pair_msg(getter=getter.name, setter=setter.name, field=field)
            violations.append(_Violation(S0501, msg, getter, info.module_path))
            violations.append(_Violation(S0501, msg, setter, info.module_path))

def _s0502_is_trivial_instance_getter(method: ast.FunctionDef, prop_name: str) -> bool:
    if len(method.body) != 1:
        return False
    stmt = method.body[0]
    if not isinstance(stmt, ast.Return) or stmt.value is None:
...
def _s0502_is_trivial_static_getter(method: ast.FunctionDef) -> bool:
    if len(method.body) != 1:
        return False
    stmt = method.body[0]
    if not isinstance(stmt, ast.Return) or stmt.value is None:
...
def _s0502_is_pure_instance_setter(setter: ast.FunctionDef, prop_name: str) -> bool:
    params = _s0501_instance_params(setter)
    if len(params) != 1:
        return False
...
def _s0502_is_pure_static_setter(setter: ast.FunctionDef) -> bool:
...
def _s0502_prefer_postsetter_msg(*, prop: str, static: bool) -> str:
...
def _s0502_is_backing_assign_to_param(stmt: ast.stmt, prop_name: str, param: str, *, static: bool) -> bool:
    if not isinstance(stmt, ast.Assign) or len(stmt.targets) != 1:
        return False
    tgt = stmt.targets[0]
    if not isinstance(tgt, ast.Attribute):
...
def _s0502_is_assign_then_extra_setter(setter: ast.FunctionDef, prop_name: str, *, static: bool) -> bool:
    """setter 体顶层的 ``self._field = value`` + 其它语句（postsetter 可合成赋值）。"""
    if static:
        if _s0502_is_pure_static_setter(setter):
            return False
        params = [a.arg for a in setter.args.args if a.arg not in ('self', 'cls')]
    else:
        if _s0502_is_pure_instance_setter(setter, prop_name):
            return False
        params = _s0501_instance_params(setter)
    if len(params) != 1:
        return False
    ...
    for stmt in setter.body:
        if _s0502_is_backing_assign_to_param(stmt, prop_name, param, static=static):
            return True
    return False

def _check_s0502_prefer_postsetter(tr: Translator, violations: list[_Violation]) -> None:
    for info in tr.classes.values():
        if info.is_protocol or info.is_descriptor or info.is_annotation:
            continue
        for prop_name, prop in info.properties.items():
            ...
            if prop.getter is None or prop.setter is None:
                continue
            if not _s0502_is_trivial_instance_getter(prop.getter, prop_name):
                continue
            if not _s0502_is_assign_then_extra_setter(prop.setter, prop_name, static=False):
                continue
            msg = _s0502_prefer_postsetter_msg(prop=prop_name, static=False)
            violations.append(_Violation(S0502, msg, prop.getter, info.module_path))
            violations.append(_Violation(S0502, msg, prop.setter, info.module_path))
        for prop_name, prop in info.static_properties.items():
            if prop.postsetter is not None:
                continue
            if prop.getter is None or prop.setter is None:
                continue
            if not _s0502_is_trivial_static_getter(prop.getter):
                continue
            if not _s0502_is_assign_then_extra_setter(prop.setter, prop_name, static=True):
                continue
            msg = _s0502_prefer_postsetter_msg(prop=prop_name, static=True)
            violations.append(_Violation(S0502, msg, prop.getter, info.module_path))
            violations.append(_Violation(S0502, msg, prop.setter, info.module_path))
...
def _s1001_group_requires_overload(defs: list[ast.FunctionDef]) -> bool:
    if len(defs) < 2:
        return False
...
def _s0401_dataclass_container_optional_msg(field: str) -> str:
    return _strict_msg(f'`{field}: list[…] = []` 等容器默认', f'`{field}: list[…] @optional = []`', '@dataclass 类体', example='``@dataclass`` 时 ``tags: list[str] @optional = []``（``char[:]/byte[:]`` 空默认同理）；``@copyable`` 选项类勿叠 ``@dataclass``，直接 ``headers: dict[str, str] = {}``（**不必** ``@optional``，见 S0402）', reason='``@dataclass`` 容器/``char[:]`` 默认须在 ``__init__`` 体内初始化，不能作 C++ 引用形参默认；``@copyable`` 走类体成员 C++ 字段初值')

def _s0402_dataclass_no_fields_msg(class_name: str) -> str:
...
def _s0402_dataclass_all_optional_msg(class_name: str) -> str:
...
def check_s0402_dataclass_required_fields(tr: Translator) -> None:
    """S0402：``@dataclass`` 须至少一个非 ``@optional`` 实例字段（须在 ``expand_dataclass`` 之前调用）。"""
    if not getattr(tr, 'strict', True):
        return
    ...
    for info in tr.classes.values():
        ...
        if not specs:
            violations.append(_Violation(S0402, _s0402_dataclass_no_fields_msg(info.name), info.node, info.module_path))
        elif all((spec.optional for spec in specs)):
            violations.append(_Violation(S0402, _s0402_dataclass_all_optional_msg(info.name), info.node, info.module_path))
    if not violations:
        return
    parts: list[str] = [f'发现 {len(violations)} 处编码规范违规（可用 --no-strict 关闭）：']
    first_loc: SourceLocation | None = None
...
def _check_s0401_dataclass_container_optional(tr: Translator, violations: list[_Violation]) -> None:
    ...
    for info in tr.classes.values():
        ...
        for spec in specs:
            ...
            violations.append(_Violation(S0401, _s0401_dataclass_container_optional_msg(spec.name), node, info.module_path))

def _s0403_final_optional_msg(field: str) -> str:
    return _strict_msg(
...
def _s0403_frozen_optional_msg(class_name: str, field: str) -> str:
    return _strict_msg(
...
def _s0404_optional_non_dataclass_msg(class_name: str, field: str) -> str:
    return _strict_msg(
...
def check_s0403_field_annotation_markers(tr: Translator) -> None:
    """S0403/S0404：字段标记组合约束（须在 ``expand_dataclass`` 之前）。"""
    if not getattr(tr, 'strict', True):
        return
    ...
    for info in tr.classes.values():
        ...
        for stmt in info.node.body:
            ...
            if 'final' in markers and 'optional' in markers:
                violations.append(
                    _Violation(S0403, _s0403_final_optional_msg(name), stmt, info.module_path),
                )
            if (not is_dc) and 'optional' in markers:
                violations.append(
                    _Violation(S0404, _s0404_optional_non_dataclass_msg(info.name, name), stmt, info.module_path),
                )
            if frozen and 'optional' in markers:
                violations.append(
                    _Violation(
                        S0403,
                        _s0403_frozen_optional_msg(info.name, name),
                        stmt,
                        info.module_path,
                    ),
                )
...
def _s0201_missing_builtins_msg(expected: str) -> str:
...
def _s0201_missing_py2cpp_star_msg() -> str:
...
def _s0201_forbidden_named_py2cpp_import_msg() -> str:
...
def _check_s0201_user_py2cpp_imports(tr: Translator, module_path: str, tree: ast.AST, violations: list[_Violation]) -> None:
    ...
    for node in ast.walk(tree):
        ...
        if mod == 'py2cpp':
            if imported and any((name in root_names for name in imported)):
                violations.append(_Violation(S0201, _s0201_forbidden_named_py2cpp_import_msg(), node, module_path))
            continue
        ...
        for sym in imported:
            ...
            if mod == 'py2cpp.test.unittest' and sym in _UNITTEST_SUBMODULE_OK:
                continue
            violations.append(_Violation(S0201, _s0201_forbidden_submodule_root_import_msg(mod, sym), node, module_path))

def _check_s0201_imports(tr: Translator, violations: list[_Violation]) -> None:
    ...
    for module_path, tree in tr.module_asts.items():
        ...
        if norm == RUNTIME_PKG or norm.startswith(f'{RUNTIME_PKG}/'):
            ...
            if expected and expected not in source:
                violations.append(_Violation(S0201, _s0201_missing_builtins_msg(expected), tree, module_path))
            continue
        from ..constant.ffi_layout import is_ffi_module_path
        if is_ffi_module_path(norm):
            ...
            if 'from py2cpp.builtins import *' not in source and 'from py2cpp import *' not in source:
                violations.append(
                    _Violation(S0201, _s0201_missing_builtins_msg('from py2cpp.builtins import *'), tree, module_path)
                )
            continue
        if star not in source:
            violations.append(_Violation(S0201, _s0201_missing_py2cpp_star_msg(), tree, module_path))
        _check_s0201_user_py2cpp_imports(tr, module_path, tree, violations)

def _s0604_forbidden_array_ann_message(root: str) -> str:
    return f'堆/栈数组类型须用切片注解（如 int[:]、int[:,:]、int[:R, :C]），勿写 {root}[…]'

def _s0604_array_ann_allowed(node: ast.Subscript) -> bool:
...
def _iter_s0604_array_ann_nodes(ann: ast.expr | None):
    if ann is None:
        return
    for node in ast.walk(ann):
        if isinstance(node, ast.Subscript) and isinstance(node.value, ast.Name):
            if node.value.id in _SLICE_ARRAY_ANN_ROOTS:
                if _s0604_array_ann_allowed(node):
                    continue
                yield node

def _check_s0604_slice_array_annotations(tr: Translator, violations: list[_Violation]) -> None:
    for module_path, tree in tr.module_asts.items():
        if not _should_check_module(tr, module_path):
            continue
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.AnnAssign):
                ...
                if node.returns is not None:
                    for hit in _iter_s0604_array_ann_nodes(node.returns):
                        violations.append(_Violation(S0604, _s0604_forbidden_array_ann_message(hit.value.id), hit, module_path))
                continue
            if ann is None:
                continue
            for hit in _iter_s0604_array_ann_nodes(ann):
                violations.append(_Violation(S0604, _s0604_forbidden_array_ann_message(hit.value.id), hit, module_path))

def _check_s1001_overload(tr: Translator, module_path: str, tree: ast.Module, violations: list[_Violation]) -> None:
    groups = _function_def_groups(tree.body)
    for name, defs in groups.items():
        if not _s1001_group_requires_overload(defs):
            continue
        ...
        for defn in defs:
            if has_named_decorator(defn, 'overload'):
                continue
            violations.append(_Violation(S1001, _strict_msg('部分 `def` 未标 `@overload`', '同名组全部 `@overload`', f'模块级同名函数 `{name}`（共 {len(defs)} 个）', example=f'在每个 `def {name}(…)` 上一行加 `@overload`', reason='静态重载须显式声明'), defn, module_path))
    for info in tr.classes.values():
        ...
        for name, defs in class_groups.items():
            ...
            for defn in defs:
                if has_named_decorator(defn, 'overload'):
                    continue
                violations.append(_Violation(S1001, _strict_msg('部分方法未标 `@overload`', '同名组全部 `@overload`', f'类 `{info.name}` 内同名方法 `{name}`（共 {len(defs)} 个）', example=f'在每个 `def {name}(…)` 上一行加 `@overload`', reason='静态重载须显式声明'), defn, module_path))
...
def _skip_class_for_s1002(info: ClassInfo) -> bool:
...
def _s1005_reverse_self_dunder_message(method_name: str) -> str:
...
def _check_s1005_reverse_self_dunder(tr: Translator, module_path: str, violations: list[_Violation]) -> None:
    ...
    for info in tr.classes.values():
        ...
        for method in _iter_class_methods(info):
            ...
            if not _reverse_binop_other_is_self_type(method, info.name):
                continue
            violations.append(_Violation(S1005, _s1005_reverse_self_dunder_message(method.name), method, module_path))

def _s1006_base_category(tr: Translator, base_ast: ast.expr) -> str:
...
def _skip_class_for_s1006(info: ClassInfo) -> bool:
...
def _check_s1006_inheritance_bases(tr: Translator, module_path: str, violations: list[_Violation]) -> None:
    for info in tr.classes.values():
        if info.module_path != module_path:
            continue
        if _skip_class_for_s1006(info):
            continue
        entity_count = 0
        seen_entity = False
        for base_ast in info.node.bases:
            cat = _s1006_base_category(tr, base_ast)
            if cat == 'skip':
                continue
            if cat == 'entity':
                entity_count += 1
                if entity_count > 1:
                    violations.append(_Violation(S1006, _strict_msg(f'class {info.name}(…)', '至多继承一个实体类', '基类列表含多个实体类', reason='Py2Cpp 不允许多继承实体类；可用多个 ``@mixin`` + 一个实体基类', example='class Host(MixinA, MixinB, Base): …'), info.node, module_path))
                    break
                seen_entity = True
                continue
            if cat == 'mixin' and seen_entity:
                violations.append(_Violation(S1006, _strict_msg(f'class {info.name}(…)', '``@mixin`` 写在实体基类之前', '实体基类之后出现 ``@mixin``', reason='mixin 须先于实体类，以便 ``__base__`` 唯一表示实体基类', example='class Host(IncMixin, Base): …'), base_ast, module_path))
                break

def _s1002_virtual_or_abstract_origin(tr: Translator, base_info: ClassInfo, base_method: ast.FunctionDef, method_name: str) -> ClassInfo | None:
...
def _append_s1002_override_violation(violations: list[_Violation], *, info: ClassInfo, method: ast.FunctionDef, module_path: str, detail: str, static: bool) -> None:
    kind = '静态方法' if static else '方法'
    violations.append(_Violation(S1002, _strict_msg(f'子类 `{info.name}.{method.name}` 覆盖{kind}', '在子类方法上加 `@override`', detail, example=f'在 `{info.name}.{method.name}` 上一行加 `@override`', reason='基类虚/纯虚或协议静态虚契约被覆盖时须显式标注，便于译期与阅读对齐 C++ ``override``'), method, module_path))

def _check_s1002_static_override(tr: Translator, info: ClassInfo, method: ast.FunctionDef, module_path: str, violations: list[_Violation], *, protocol_static: dict[str, dict[str, str]], class_bindings: dict[str, set[str]]) -> None:
    ...
    if detail is not None:
        _append_s1002_override_violation(violations, info=info, method=method, module_path=module_path, detail=detail, static=True)

def _check_s1002_instance_override(tr: Translator, info: ClassInfo, method: ast.FunctionDef, module_path: str, violations: list[_Violation]) -> None:
    if has_named_decorator(method, 'staticmethod'):
        return
    if has_named_decorator(method, 'override'):
        return
    ...
    if base_info.is_mixin:
        return
    origin = _s1002_virtual_or_abstract_origin(tr, base_info, base_method, method.name)
    if origin is None:
        return
    ...
    _append_s1002_override_violation(violations, info=info, method=method, module_path=module_path, detail=f'继承链上 `{origin.name}.{method.name}` 已标 {origin_tag}。', static=False)

def check_static_virtual_override_s1002(tr: Translator) -> None:
    """静态虚 ``@override``（**S1002**）：全模块含 ``test/fail/``。"""
    violations: list[_Violation] = []
    for module_path in tr.module_asts:
        skip = getattr(tr, 'skip_cached_analysis_module', None)
        if skip is not None and skip(module_path):
        ...
        for info in tr.classes.values():
            ...
            for method in _iter_class_methods(info):
                ...
                if _is_property_accessor(method):
                    continue
                _check_s1002_static_override(tr, info, method, module_path, violations, protocol_static=protocol_static, class_bindings=class_bindings)
    if not violations:
        return
    parts: list[str] = [f'发现 {len(violations)} 处静态虚 ``@override`` 违规（S1002）：']
    first_loc = None
    for v in violations:
        loc = location_from_node(tr, v.node, module_path=v.module_path)
        prefix = loc.prefix() if loc is not None else '?'
...
def _check_s1002_override_virtual(tr: Translator, module_path: str, violations: list[_Violation]) -> None:
    for info in tr.classes.values():
        if info.module_path != module_path:
            continue
        if _skip_class_for_s1002(info):
        ...
        for method in _iter_class_methods(info):
            ...
            if _is_property_accessor(method):
                continue
            _check_s1002_instance_override(tr, info, method, module_path, violations)
...
def _check_s1003_method_decorator_conflicts(tr: Translator, module_path: str, violations: list[_Violation]) -> None:
    for info in tr.classes.values():
        ...
        for method in _iter_class_methods(info):
            ...
            for a, b, reason in _S1003_FORBIDDEN_DECORATOR_PAIRS:
                if a in decs and b in decs:
                    if info.is_protocol and a == 'abstract' and (b == 'staticmethod'):
                        continue
                    violations.append(_Violation(S1003, _strict_msg(f'`{info.name}.{method.name}` 装饰器冲突', f'移除 ``@{a}`` 或 ``@{b}``', reason, example=f'``@{a}`` + ``@{b}`` 不可同用（类比 C# 修饰符）'), method, module_path))
                    break
        ...
        for method in _iter_class_methods(info):
            ...
            if info.is_protocol:
                if 'staticmethod' in decs and (not is_static_virt):
                    violations.append(_Violation(S1003, _strict_msg(f'`{info.name}.{method.name}` 协议静态成员', '``@protocol`` 内 ``@staticmethod`` 须与 ``@virtual`` 或 ``@abstract`` 同用', '静态虚契约仅用于协议编译期派发'), method, module_path))
                if ('virtual' in decs or 'abstract' in decs) and 'staticmethod' not in decs:
                    violations.append(_Violation(S1003, _strict_msg(f'`{info.name}.{method.name}` 协议虚成员', '``@protocol`` 内 ``@virtual``/``@abstract`` 实例方法须写 ``...`` 桩体；静态契约须 ``@staticmethod`` + ``@virtual``/``@abstract``', '实例虚表与静态虚契约语义不同'), method, module_path))
                continue
            if is_static_virt:
                violations.append(_Violation(S1003, _strict_msg(f'`{info.name}.{method.name}` 静态虚方法', '``@staticmethod`` + ``@virtual``/``@abstract`` 仅允许写在 ``@protocol`` 内', '实体类静态方法无运行期虚派发'), method, module_path))

def _s1004_unused_type_param_message(name: str) -> str:
...
def _check_s1004_unused_pep695_type_params(tree: ast.Module, module_path: str, violations: list[_Violation]) -> None:
    ...
    for node in ast.walk(tree):
        ...
        for name in declared:
            if name not in used:
                violations.append(_Violation(S1004, _s1004_unused_type_param_message(name), node, module_path))
...
class _StrictStyleChecker(ast.NodeVisitor):
    ...
    def _s0303_construct_rule(self) -> str:
    ...
    def _s0303_self_ann_rule(self) -> str:
        return S0304

    def _s0303_scene(self) -> str:
        return _s0303_scene_label(aug_op=self._current_aug_op, in_aug_assign=self._context_has('aug_assign'), in_binop=self._context_has('binop'), in_call_arg=self._context_has('call_arg'), in_new_preferred=self._in_new_preferred_context())
    ...
    def _check_s0102_cpp_style_name(self, name: str, node: ast.AST) -> None:
        if _is_dunder(name):
            return
        if self._class_stack:
            allowed = _S0102_CLASS_METHOD_ALLOW.get(self._class_stack[-1])
            if allowed is not None:
                key = name.lstrip('_') if name.lstrip('_') else name
                if name in allowed or key in allowed:
                    return
        msg = _s0102_cpp_style_message(name)
        if msg is None:
            return
        self._add(S0102, node, msg)

    def _check_s0106_translator_only_member(self, name: str, node: ast.AST, *, kind: str) -> None:
        if not self._class_stack:
            return
        if _is_dunder(name):
            return
        if name not in _S0106_RESERVED_MEMBER_NAMES:
            return
        class_name = self._class_stack[-1]
        self._add(S0106, node, _s0106_translator_only_member_message(name, class_name, kind=kind))

    def _check_s0603_tuple_annotation(self, ann: ast.expr | None, node: ast.AST) -> None:
        ...
        if hint is None:
            return
        self._add(S0603, sub, _strict_msg('`tuple[…]`', hint, '最外层元组类型注解处', example=f'`x: {hint} = …` 勿 `x: tuple[…] = …`（内层 `list[tuple[K,V]]` 可保留）', reason='PEP 695 元组类型简写须用括号形式'))

    def _check_s0602_annotation(self, ann: ast.expr | None, node: ast.AST) -> None:
        ...
        if hit is None:
            return
        if _s0602_allows_class_type_reference(hit, class_name, self._class_type_params):
            return
        self._add(S0602, hit, _s0602_same_class_message(class_name, scene='类型注解'))

    def _check_s0602_same_class_expr(self, node: ast.Attribute) -> None:
        if not self._class_stack:
            return
        class_name = self._class_stack[-1]
        if _is_desugar_generated_name(class_name):
        ...
        if not isinstance(node.value, ast.Name) or node.value.id != class_name:
            return
        self._add(S0602, node, _s0602_same_class_message(class_name, scene='表达式', attr=node.attr))

    def _check_s0303_prefer_new_staticproperty(self, node: ast.Attribute) -> None:
        ...
        if info is None or node.attr not in info.static_properties:
            return
        if _s0303_exempt_new_receiver_class(info):
            return
        if not _ann_is_self(self._type_context_ann):
            if _ann_root_name(self._type_context_ann) != info.name:
                return
        bad = f'Self.{node.attr}'
        self._add(S0304, node, _s0303_msg_prefer(bad, f'new.{node.attr}', self._s0303_scene(), example=f'`x: Self = new.{node.attr}` 勿 `{bad}`', reason='``new`` 指赋值/返回注解的类型；注解为本类时 ``@staticproperty`` 须 ``new.属性`` 勿 ``Self.属性``'))

    def visit_Attribute(self, node: ast.Attribute):
        self._check_s0602_same_class_expr(node)
        self._check_s0303_prefer_new_staticproperty(node)
        if _is_new_receiver_attribute(node) and (self._context_has('binop') or self._context_has('aug_assign')):
            self._add(S0306, node, _s0303_msg_prefer(f'new.{node.attr}', f'Self(...) 或显式 Cls.{node.attr}', self._s0303_scene(), example=f'`rotation * Vector3.{node.attr}`、`out += Self(x)` 勿 `rotation * new.{node.attr}`', reason=_s0303_reason_no_new_in_expr()))
        self.generic_visit(node)
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        for tp in getattr(node, 'type_params', None) or ():
            if isinstance(tp, ast.TypeVar) and _is_short_class_type_param_name(tp.name):
                self._add(S0105, tp, _s0105_short_class_type_param_message(node.name, tp.name))
    ...
    def _s1101_exempt(self) -> bool:
        return _s1101_module_exempt(self.module_path)

    def _check_s1101_refcount_use(self, node: ast.AST, *, form: str, example: str) -> None:
        if self._s1101_exempt():
            return
        self._add(S1101, node, _strict_msg(form, 'T / new()', '源码', reason='``RefCount`` 为基础设施类型名，用户代码须写 ``@refcount`` 类名 ``T``（或 ``T: refcount`` 形参）；清空强引用用 ``x = new()``，勿 ``RefCount()``', example=example))

    def _check_s1101_refcount_subscript(self, node: ast.Subscript) -> None:
        if isinstance(node.value, ast.Name) and node.value.id == 'RefCount':
            self._check_s1101_refcount_use(node, form='RefCount[…]', example='got: n: Node = new()')

    def _check_s1101_refcount_call(self, node: ast.Call) -> None:
        if isinstance(node.func, ast.Name) and node.func.id == 'RefCount':
            self._check_s1101_refcount_use(node, form='RefCount(…)', example='got: n = new()')

    def _check_s0103_thin_wrapper(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
        class_methods: set[str] | None = None
        if self._class_stack:
            class_methods = self._class_methods.get(self._class_stack[-1])
        msg = _s0103_try_thin_param_forward(node, class_methods=class_methods, module_funcs=self._module_func_names, self_like=self._self_like_params)
        if msg is not None:
            self._add(S0103, node, msg)
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        if _is_dunder(node.name):
            ...
            self._check_s0102_cpp_style_name(node.name, node)
            if self._class_stack:
                self._check_s0106_translator_only_member(node.name, node, kind='方法')
        for dec in node.decorator_list:
            self.visit(dec)
        for arg in node.args.args:
            if arg.arg not in ('self', 'cls'):
                self._check_s0602_annotation(arg.annotation, arg)
                self._check_s0603_tuple_annotation(arg.annotation, arg)
        self._check_s0602_annotation(node.returns, node)
        self._check_s0603_tuple_annotation(node.returns, node)
        ...
        self._current_func_sig = _s0901_sig_for_func_node(self, node)
        self._visit_function_defaults(node)
        self._check_s0103_thin_wrapper(node)
        for stmt in node.body:
            self.visit(stmt)
        _s0801_scan_function_body(self, node.body, func=node)
    ...
    def visit_TypeAlias(self, node: ast.TypeAlias):
        ...
        if self._class_stack and self._class_type_params:
            if isinstance(node.value, ast.Name) and node.value.id in self._class_type_params:
                ...
                self._add(S0605, node, f'类形参 ``{node.value.id}`` 已自动生成 ``using {node.name} = _{node.name}``；勿手写 ``type {node.name} = {node.value.id}``，请改为 {hint}')
        self._check_s0602_annotation(node.value, node)
        self._check_s0603_tuple_annotation(node.value, node)
        self.generic_visit(node)
        self._scope_type_params = prev_scope
    ...
    def _s0101_allows_dunder_call(self, dunder: str) -> bool:
        if dunder in _S0101_GLOBAL_DUNDERS:
            return True
    ...
    def _s0303_cls_call_in_new_context(self, node: ast.Call, info: ClassInfo) -> bool:
    ...
    def _s0303_explicit_ctor_example(self, node: ast.Call, info: ClassInfo) -> str:
    ...
    def _check_s0303_prefer_new_receiver_static(self, node: ast.Call) -> None:
        ...
        if isinstance(type_expr, ast.Name) and type_expr.id == 'Self' and _ann_is_self(ann):
            ...
            if info is not None:
                sig = info.method_sigs.get(method)
                if sig is not None and sig.is_static and (not _s0303_exempt_new_receiver_class(info)):
                    bad = f'Self.{method}(...)'
                    self._add(S0304, node, _s0303_msg_prefer(bad, f'new.{method}(...)', self._s0303_scene(), example=f'`x: Self = new.{method}(...)` 勿 `{bad}`', reason='注解/返回为 `Self` 时静态工厂须 `new.方法` 勿 `Self.方法`'))
            return
        ...
        if ann_info is not None:
            if isinstance(type_expr, ast.Name):
                ...
                if union_accepts_case_union(ann_info, union_name) and method in union_variant_names(ann_info):
                    ...
                    self._add(S0304, node, _s0303_msg_prefer(bad, f'new.{method}(...)', self._s0303_scene(), example=f'`x: {ann_text} = new.{method}(...)` 勿 `{bad}`', reason='注解已给出 @union 类型，变体构造须 `new.变体` 勿显式 `Union.变体`'))
                    return
        ...
        if isinstance(type_expr, ast.Subscript):
            ...
            if info is None or info.type_params or _s0303_exempt_new_receiver_class(info):
                return
        else:
            return
        if _static_method_on_class(self.tr, type_expr, method) is None:
            return
        bad = f'{ast.unparse(type_expr)}.{method}(...)'
        self._add(S0304, node, _s0303_msg_prefer(bad, f'new.{method}(...)', self._s0303_scene(), example=f'`x: {ann_text} = new.{method}(...)` 勿 `{bad}`', reason='注解已给出目标类，静态工厂/方法须 `new.方法` 勿显式 `Cls.方法`/`Cls[T].方法`/`Self.方法`'))

    def _check_s0303_prefer_new_union_match(self, node: ast.Match) -> None:
        from .union_expand import union_accepts_case_union, union_variant_names
        subject_union = _match_subject_union_info(self, node)
        if subject_union is None:
            subject_union = _s0804_union_info_for_match(self, node)
        if subject_union is None:
            return
        for case in node.cases:
            if is_wildcard_pattern(case.pattern):
                continue
            for branch_pat in _s0803_union_branch_patterns(case.pattern):
                if not _s0803_union_case_uses_explicit_union_syntax(branch_pat):
                    continue
                ref = _s0803_union_case_ref(branch_pat)
                if ref is None:
                    continue
                ...
                self._add(S0304, case, _s0303_msg_prefer(bad, f'new.{variant}', 'match @union', example=f'`case new.{variant}:` / `case new.{variant}(...):` 勿 `case {bad}:`', reason='match 主体已是 @union，case 须 `new.变体` 勿显式 `Union.变体`'))

    def _s0303_in_desugar_host(self) -> bool:
        if not self._class_stack:
            return False
        return _is_desugar_generated_name(self._class_stack[-1])

    def _s0303_call_exempt(self, node: ast.Call) -> bool:
        if self._s0303_in_desugar_host():
            return True
        name = _call_target_name(node.func)
        return _is_desugar_generated_name(name)

    def _check_s0303_new_str_bytes_literal(self, node: ast.Call) -> bool:
        ...
        if len(node.args) == 1 and isinstance(node.args[0], ast.Constant):
            ...
            if isinstance(val, str) and _ann_root_name(ann) == 'str':
                self._add(S0303, node, _s0303_priority_message('""', scene=self._s0303_scene(), example='`s: str = "hi"` 勿 `s: str = new("hi")`'))
                return True
            if isinstance(val, bytes) and _ann_root_name(ann) == 'bytes':
                self._add(S0303, node, _s0303_priority_message('b""', scene=self._s0303_scene(), example='`b: bytes = b"\\x00"` 勿 `b: bytes = new(b"\\x00")`'))
                return True
        return False
    ...
    def _check_s0303_construct_priority(self, node: ast.Call) -> None:
        ...
        if _is_new_call(node) and (not node.args) and (not node.keywords):
            ...
            if ann_root in _NO_EMPTY_MAKE_ANN_ROOTS:
                self._add(S0303, node, _s0303_no_empty_new_message(ann_root))
                return
        hint = _s0303_preferred_init_hint(ann)
        ...
        if host_hint is not None:
            ...
            if _is_new_call(node) and (not node.args) and (not node.keywords):
                if host_hint != 'new()':
                    self._add(S0303, node, _s0303_priority_message(host_hint, scene=f'`{host}` 类内 `-> Self` 的{self._s0303_scene()}', example=f'`return {host_hint}` 或 `out: Self = {host_hint}` 勿 `new()`'))
                return
            if func_name == 'Self' and (not node.args) and (not node.keywords):
                self._add(S0303, node, _s0303_priority_message(host_hint, scene=f'`{host}` 类内 `-> Self` 的{self._s0303_scene()}', example=f'`return {host_hint}` 勿 `return Self()`'))
                return
        if func_name in _EMPTY_SET_FACTORIES and (not node.args) and (not node.keywords):
            self._add(S0303, node, _strict_msg('`set()` / `frozenset()`', 'new()', self._s0303_scene(), example='`s: set[int] = new()` 勿 `s = set()`', reason='空集合无 `{}` 字面量，须靠类型上下文 `new()`'))
            return
        if hint == 'new()':
            if _is_new_call(node) and (not node.args) and (not node.keywords):
                return
            if func_name == 'Self' and (not node.args) and (not node.keywords) and (not _ann_is_self(ann)):
                self._add(S0303, node, _strict_msg('Self()', 'new()', self._s0303_scene(), example='`fs: frozenset[T] = new()` 勿 `fs = Self()`', reason='空 set/frozenset 无字面量捷径'))
            return
        if _is_new_call(node) and (not node.args) and (not node.keywords):
            if hint is not None:
                self._add(S0303, node, _s0303_priority_message(hint, scene=self._s0303_scene()))
                return
        if func_name == 'Self' and (not node.args) and (not node.keywords) and (not _ann_is_self(ann)):
            if hint is not None:
                self._add(S0303, node, _s0303_priority_message(hint, scene=self._s0303_scene()))
            else:
                self._add(S0303, node, _s0303_msg_prefer('Self()', 'new() 或容器/str 字面量', self._s0303_scene(), example='`return new()`、`scratch: list[T] = []`', reason='无更具体的类型字面量捷径'))
            return
        if func_name == 'Self' and (not node.args) and (not node.keywords) and _ann_is_self(ann) and (not self._in_self_literal_host_class()):
            self._add(S0304, node, _s0303_msg_prefer('Self()', 'new()', self._s0303_scene(), example='`return new()` 或 `z: Self = new()`（mixin/泛型类无 `[]`/`""` 宿主）', reason='注解为 `Self` 且非 str/bytes/list 等具体宿主类'))
            return
        if func_name == 'Self' and _ann_is_self(ann) and (node.args or node.keywords) and (not self._context_has('aug_assign')) and (not self._context_has('binop')):
            self._add(S0304, node, _s0303_msg_prefer('Self(...)', 'new(...)', self._s0303_scene(), example='`return new(buf)`、`out: Self = new(x)` 勿 `return Self(x)`', reason='左侧/返回注解为 `Self` 时有类型上下文，`new` 优先'))
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == 'ord':
            if not _s0801_is_ord_single_char(node):
                try:
                    bad = ast.unparse(node)
                except Exception:
                    bad = 'ord(...)'
                self._add(S0902, node, _s0902_ord_non_literal_msg(bad))
        s0706 = _s0706_try_range_redundant_form(node)
        if s0706 is not None:
            self._add(S0706, node, s0706)
        if isinstance(node.func, ast.Name) and node.func.id in ('__cmp__', '__mod__', '__truediv__', '__floordiv__'):
            ...
            from ..analysis.proxy import is_s0101_init_forward_call, is_super_dunder_call
            in_init = self._current_method == '__init__' and bool(self._class_stack)
            if not is_super_dunder_call(node) and (not is_s0101_init_forward_call(node, in_class_init=in_init)) and (not self._s0101_allows_dunder_call(node.func.attr)):
                self._add('S0101', node, _s0101_dunder_call_message(node.func.attr))
        if isinstance(node.func, ast.Subscript) and isinstance(node.func.value, ast.Name) and (node.func.value.id in _DEDUCED_MEMORY_FUNCS):
            ...
            self._add(S0308, node, _strict_msg(f'`{fn}[T](…)`', f'`{fn}(…)`', '内存/指针辅助调用处', example=f'`destroy(ptr)`、`freeArray(arr)` 勿 `{fn}[T](ptr)`', reason='C++ 侧从指针/实参推导模板实参'))
        func_name = _call_target_name(node.func)
        if func_name in _EMPTY_CONTAINER_FACTORIES and (not node.args) and (not node.keywords):
            self._add(S0301, node, _s0301_empty_factory_message(func_name))
        ...
        if _is_new_call(node) and (self._context_has('aug_assign') or self._context_has('binop')):
            ...
            self._add(self._s0303_construct_rule(), node, _s0303_msg_prefer('new(...)', 'Self(...) 或显式 `Cls(...)` / `Cls.静态成员`', scene, example=example, reason=_s0303_reason_no_new_in_expr()))
        if isinstance(node.func, ast.Attribute) and _is_new_receiver_attribute(node.func) and (self._context_has('aug_assign') or self._context_has('binop')):
            attr = node.func.attr
            self._add(S0306, node, _s0303_msg_prefer(f'new.{attr}(...)', f'Self.{attr}(...) 或显式 Cls.{attr}(...)', self._s0303_scene(), example=f'`long._one() + long._one()`、`rotation * Vector3.right` 勿 `new.{attr}() + new.{attr}()`', reason=_s0303_reason_no_new_in_expr()))
        self._check_s0303_construct_priority(node)
        self._check_s0303_prefer_new_receiver_static(node)
        if isinstance(node.func, ast.Subscript) and isinstance(node.func.value, ast.Name) and (node.func.value.id == 'cast') and self._in_new_preferred_context() and (self._type_context_ann is not None) and node.args:
            ...
            if _expr_same(cast_ty, ctx_ty):
                ...
                self._add(S0309, node, _strict_msg(bad, 'cast(...)', self._s0303_scene(), example=f'`x: {ast.unparse(ctx_ty)} = cast(obj)` 勿 `{bad}`', reason='赋值/返回注解已给出目标类型，须 `cast(obj)` 简写'))
        info = _class_info_for_ctor(self.tr, node.func)
        if info is not None and info.name != 'long' and (not self._s0303_call_exempt(node)) and self._in_new_preferred_context() and self._s0303_cls_call_in_new_context(node, info) and (self._dunder_method not in ('__copy__', '__move__')) and (id(node) not in self._kwargs_opts_ctor_ids) and (not self._context_has('binop')) and (not self._context_has('aug_assign')):
            ann_cls = _ann_root_name(self._type_context_ann)
            if ann_cls == info.name and node.args and (not node.keywords):
                ...
                self._add(S0304, node, _s0303_msg_prefer(bad, 'new(...)', self._s0303_scene(), example=self._s0303_explicit_ctor_example(node, info), reason='返回/赋值注解已给出目标类，须 `new` 勿显式 `Cls`/`Cls[T]` 构造'))
            else:
                self._add(S0303, node, _s0303_msg_prefer(f'{info.name}(...)', 'new(...)', self._s0303_scene(), example=f'`x: {info.name} = new(field=v)` 勿 `{info.name}(field=v)`', reason='初始化/返回有类型注解时 `new` 优先于显式类名构造'))
        deleg = _delegate_info_for_ctor(self.tr, node.func)
        if deleg is not None and (not self._s0303_call_exempt(node)) and self._in_new_preferred_context() and (not node.args) and (not node.keywords) and (self._dunder_method not in ('__copy__', '__move__')):
    ...
    def _s0405_check_target(self, target: ast.expr, node: ast.AST) -> None:
        ...
        if not _is_self_field_ann_assign(target):
            return
        info = self._current_class_info()
        if info is None or _s0405_skip_class(info):
            return
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign):
        if self._class_stack and self._current_method is None:
            ...
            if field:
                self._check_s0106_translator_only_member(field, node, kind='字段')
        if self._current_method is not None and self._current_method != '__init__' and _is_self_field_ann_assign(node.target) and (not self._s0303_in_desugar_host()):
        ...
        if isinstance(node.value, ast.BinOp) and type(node.value.op) in _AUG_BINOPS:
            for target in node.targets:
                if _expr_same(target, node.value.left):
                    self._add(S0704, node, _s0704_binop_assign_message(node))
                    break
        if len(node.targets) == 1 and isinstance(node.targets[0], ast.Tuple):
            _s1202_check_tuple_unpack(self, node)
        if not self._context_has('call_arg'):
            prev_ann = self._type_context_ann
            field_ann: ast.expr | None = None
            if len(node.targets) == 1 and (not self._s0303_in_desugar_host()):
    ...
    def visit_Match(self, node: ast.Match):
        _s0803_check_match_default_case(self, node)
        _s0805_check_optional_match_patterns(self, node)
        _s0804_check_union_case_order(self, node)
        self._check_s0303_prefer_new_union_match(node)
        self._push('match')
    ...
    def visit_Compare(self, node: ast.Compare):
        ...
        _s0805_check_optional_identity_compare(self, node)
        if _s0701_is_len_zero_compare(node) and self._current_method != '__bool__':
            self._add(S0701, node, _s0701_len_zero_suggestion(node))
        self.generic_visit(node)

    def visit_Subscript(self, node: ast.Subscript):
        if _slice_has_zero_lower(node.slice):
            self._add(S0702, node, _s0702_zero_slice_message(node))
        k: int | None = _s0703_is_len_minus_k_subscript(node)
        if k is not None and isinstance(node.value, ast.Name):
            self._add(S0703, node, _s0703_len_minus_k_subscript_suggestion(node.value.id, k))
        self.generic_visit(node)
