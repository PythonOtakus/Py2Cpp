...
PY2CPP_IGNORE
#include "py2cpp/console/popen.h"
...
static void _popen_append_bytes(PyArray<PyChar>& codes, const char* p, int n)
{
  if ((!p) || n <= 0)
  {
    return;
...
static PyStr _popen_codes_to_str(PyArray<PyChar>& codes)
{
  if (codes.__len__() <= 0)
  {
    return PyStr("");
...
#if defined(_WIN32)
static wchar_t* _popen_utf8_to_wide(const PyStr& s)
{
  char buf[32768];
...
static void _popen_close_handle(HANDLE& h)
{
...
static HANDLE _popen_nul_handle(DWORD access)
{
...
static void _popen_append_quoted(wchar_t* dst, int& at, const wchar_t* src, int cap)
{
  int need_quote = 0;
  for (int i = 0; src[i]; i++)
  {
...
static wchar_t* _popen_env_block(const PyDict<PyStr, PyStr>& env)
{
  PyInt n = env.__len__();
  if (n <= 0)
  {
  ...
  for (PyInt i = 0; i < n && at + 4 < 65536; i++)
  {
    ...
    wchar_t* wk = _popen_utf8_to_wide(key);
    wchar_t* wv = _popen_utf8_to_wide(val);
    if ((!wk) || (!wv))
    {
...
static void _popen_drain_avail(HANDLE h, PyArray<PyChar>& codes)
{
  if (!h)
  {
    return;
  ...
  for (;;)
  {
    ...
    _popen_append_bytes(codes, stack, (int)n);
  }
}

static PyStr _popen_read_rest(HANDLE h)
{
  ...
  return _popen_codes_to_str(codes);
}
#endif
...
)
{
  ...
  PyPopenState* st = reinterpret_cast<PyPopenState*>(static_cast<uintptr_t>(::ffi::crt::stdlib::pyiCalloc(1, sizeof(PyPopenState))));
  if (!st)
  {
...
PyPopen::PyPopen(PyPopen&& other)
{
  this->_state = other._state;
  other._state = 0;
}

PyPopen& PyPopen::operator=(PyPopen&& other)
{
  if (this != &other)
  {
    this->~PyPopen();
...
PyPopen::~PyPopen()
{
  ...
  PyPopenState* st = (PyPopenState*)(uintptr_t)_state;
#if defined(_WIN32)
  if (st->pi.hProcess)
  {
  ...
  _popen_close_handle(st->out_rd);
  _popen_close_handle(st->out_wr);
  _popen_close_handle(st->err_rd);
  _popen_close_handle(st->err_wr);
  _popen_close_handle(st->in_rd);
  _popen_close_handle(st->in_wr);
#else
  if (st->out_rd >= 0)
  {
    close(st->out_rd);
...
void PyPopen::start()
{
  ...
  for (int i = 0; i < n; i++)
  {
    wchar_t* w = _popen_utf8_to_wide(st->args.__getitem__(i));
    if (!w)
    {
      throw PY2CPP_TYPE(PyOSError)();
    }
    ...
    _popen_append_quoted(cmdline, at, w, 32768);
  ...
  {
    si.hStdInput = _popen_nul_handle(GENERIC_READ);
  }
  else
  {
  ...
  {
    si.hStdOutput = _popen_nul_handle(GENERIC_WRITE);
  }
  else
  {
  ...
  {
    si.hStdError = _popen_nul_handle(GENERIC_WRITE);
  }
  else
  {
  ...
  if (st->cwd.__len__() > 0)
  {
    wcwd = _popen_utf8_to_wide(st->cwd);
  }
  wchar_t* wenv = _popen_env_block(st->env);
  DWORD flags = 0;
  if (wenv)
  {
...
PyPopen& PyPopen::__enter__()
{
  this->start();
  return *this;
}

void PyPopen::__exit__()
{
...
PyInt PyPopen::wait(PyFloat64 timeout)
{
  PyPopenState* st = (PyPopenState*)(uintptr_t)_state;
  if ((!st) || (!st->started))
  {
...
PyCompletedProcess PyPopen::communicate(PyStr input, PyFloat64 timeout)
{
  PyPopenState* st = (PyPopenState*)(uintptr_t)_state;
  if ((!st) || (!st->started))
  {
  ...
  for (;;)
  {
    _popen_drain_avail(st->out_rd, out_codes);
    _popen_drain_avail(st->err_rd, err_codes);
    DWORD PySlice = 10;
  ...
  st->exit_code = code;
  st->done = TRUE;
  PyStr out = _popen_codes_to_str(out_codes);
  PyStr err = _popen_codes_to_str(err_codes);
  if (st->out_rd)
  {
    PyStr rest = _popen_read_rest(st->out_rd);
    if (rest.__len__() > 0)
    {
  ...
  if (st->err_rd)
  {
    PyStr rest = _popen_read_rest(st->err_rd);
    if (rest.__len__() > 0)
    {
  ...
  if (st->out_rd >= 0)
  {
    for (;;)
    {
      ...
      _popen_append_bytes(out_codes, stack, n);
    }
  }
  if (st->err_rd >= 0)
  {
    for (;;)
    {
      ...
      _popen_append_bytes(err_codes, stack, n);
    }
  }
  return PyCompletedProcess(
    st->args, (PyInt)st->exit_code, _popen_codes_to_str(out_codes), _popen_codes_to_str(err_codes)
  );
#endif
}

PyInt PyPopen::returnCode__get() const
{
  PyPopenState* st = (PyPopenState*)(uintptr_t)_state;
  if ((!st) || (!st->done))
  {
...
PyBool PyPopen::running__get() const
{
  PyPopenState* st = (PyPopenState*)(uintptr_t)_state;
