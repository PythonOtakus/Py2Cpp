"""列出 ``examples/**/*.py`` 中匹配模式的相对路径（供 ``demo.bat``）。"""
...
def main(argv: list[str] | None = None) -> int:
  ...
  if not args:
    print(
      "用法: match_demo_files.py PATTERN [PATTERN ...]\n"
      "  无通配符: 路径/文件名子串（不区分大小写），如 panel\n"
      "  含 * ?: fnmatch，如 *panel*.py",
      file=sys.stderr,
    )
    return 2
  root = pathlib.Path(__file__).resolve().parent.parent / "examples"
  if not root.is_dir():
    print(f"ERROR: missing examples dir: {root}", file=sys.stderr)
    return 2
  matched: list[str] = []
  seen: set[str] = set()
