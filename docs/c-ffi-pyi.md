...
状态：**生成器 + 译器识别 + sqlite glue 试点已通**（`sqlite3.h` → `ffi/sqlite/sqlite3.pyi` → `ffi::sqlite::sqlite3`；句柄 `sqlite3_h` 等 `*_h` 别名；`.inl` 转发；`+sqlite.inl` 经 FFI 调 C）。
...
- 不透明句柄：**`uint64`**（结构体指针）或 **`uintptr`**（`void*` / 函数指针）
...
| C | `.pyi` |
|---|--------|
| `sqlite3*` / 其它不完整 `struct X*` | 句柄别名（`type sqlite3_h = uint64`；后缀 `_h`，**永不**与 C 标签同名） |
| `sqlite3**`（出参） | `Pointer[uint64]` |
| （glue） | `struct X*` 用 **C 标签**（由 `ffi_opaque_c_tag` 去掉 `_h`） |
| `void*` | `uintptr` |
| 函数指针 / 回调 typedef | `uintptr` |
| `const char*` / `char*` | `c_str` |
| `int` / `unsigned` / 定宽整数 | `int` / `uint` / `int64` / `uint64` |
| `float` / `double` | `float64` |
| `T*`（标量/完整类型） | `Pointer[T]`（`T` 已映射） |
| `void` 返回 | `None` |
| `va_list` 等难映射 | `uintptr` + 行尾注释 |

不生成完整 `struct` 字段布局（VFS/IO methods 等）：指针一律按句柄 / `uintptr`，避免把 C ABI 结构体铺进 Py2Cpp。
...
- **其它头（含 Win32）**：收字面量对象宏，跳过空/属性宏、编译器内建前缀（`__`、`_MSC_`…）及 `WINAPI`/`CALLBACK` 等。
...
- 段落顺序：句柄 `type` 别名 → 常量 → `@native` 函数

**统一输出根目录 `ffi/`**（与源 `.h` 路径对应，不写回 `third_party/` 旁）：
...
2. **显式 import**：`import ffi.windows as win` 或 `from ffi.windows import CreateWindowExW, WM_PAINT`。
3. **C++ 命名空间隔离**：落到独立空间（`ffi::windows` / `ffi::sqlite::sqlite3` 等，**不**挂 `py2cpp::`；路径段 = 命名空间段；句柄别名 `*_h` 避免与 C 标签撞名），**不**进 `minimal.h` 默认路径。
4. **业务零 re-export 全量**：`py2cpp/ui/...` 只拉所需叶子；用户只碰业务 API。
5. **sqlite 天然前缀**（`sqlite3_*`）隔离较好；Win32 无统一前缀，更依赖命名空间 + 显式 import。

**一般不推荐**：给全部符号加 `Win_` 前缀；把全量 `windows.pyi` 塞进 runtime umbrella。
...
| 项 | 行为 |
|----|------|
| Import | `from ffi.sqlite.sqlite3 import sqlite3_open, SQLITE_OK` / `import ffi.windows as win` |
| 源文件 | 仓库根 `ffi/**/*.pyi`（固定相对仓库根，与入口 `project_root` 无关） |
...
| 句柄别名 | `*_h`（`sqlite3_h` / `sqlite3_stmt_h`）；glue 用 C 标签 `struct sqlite3*` |
...
| `@native` glue | `src/emit/ffi_glue_emit.py`：`inline` 转发；`#include <c_header>`（尖括号，防同目录自包含）；`struct T*` 句柄转换；白名单 `ffi_glue_allowlist` |
| sqlite 业务 | `py2cpp/sql/sqlite.py` import 闭包拉 FFI；`templates/sql/+sqlite.inl` 调 `::ffi::sqlite::sqlite3::…` |
