"""``varint`` 的 ``//`` / ``%`` / ``/`` 须走实例 dunder，勿误用 ``PyInt`` 全局算子。"""
...
class VarintBinopEmitTests(unittest.TestCase):
...
{body}"""
  ...
  def test_floordiv_varint_dunder(self):
    cpp = self._translate(
      "  a: varint = 7\n  b: varint = 3\n  q: varint = a // b\n  return int(q)\n",
    )
  ...
  def test_mod_varint_dunder(self):
    cpp = self._translate(
      "  a: varint = 7\n  b: varint = 3\n  r: varint = a % b\n  return int(r)\n",
    )
  ...
  def test_truediv_varint_dunder(self):
    cpp = self._translate(
      "  a: varint = 7\n  b: varint = 3\n  return a / b\n",
    )
